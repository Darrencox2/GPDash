...
export function getSiteUrl() {
  ...
  if (typeof window !== 'undefined' && window.location?.origin) {
    // Transient Vercel per-deployment hosts (e.g. gpdash-7xdj2.vercel.app) stop
    // resolving once newer deploys supersede them, so a link baked into an email
    // 404s when opened later. If we're on one of those, prefer the stable
    // preview alias so reset/invite links keep working even without the env var.
    if (/\.vercel\.app$/i.test(window.location.hostname)) return 'https://preview.gpdash.net';
