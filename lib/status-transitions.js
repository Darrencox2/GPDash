...
export const STATUS_TRANSITIONS = {
  left_winddown: {
    key: 'left_winddown',
    label: 'Has left',
    defaultWeeks: 8, // the usual run for results to stop arriving; the
                     // dialog takes an explicit date, this only seeds it
...
export function applyTransition(data, clinicianId, transitionKey, { weeks, untilDate, startDate: startOverride, by } = {}) {
  const t = STATUS_TRANSITIONS[transitionKey];
  if (!t) throw new Error(`Unknown transition: ${transitionKey}`);
  const start = startOverride ? new Date(startOverride + 'T12:00:00') : new Date();
  // Dates first (Darren: "use dates rather than 6 weeks"). An explicit
  // untilDate wins; the weeks path survives for old callers and as the
  // fallback that seeds the date input's default.
  let end;
  if (untilDate) {
    end = new Date(untilDate + 'T12:00:00');
    if (!(end > start)) end = new Date(start.getTime() + WEEK_MS);
  } else {
    const w = Math.max(1, Math.min(52, Number(weeks) || t.defaultWeeks));
    end = new Date(start.getTime() + w * WEEK_MS);
  }
  ...
  const absence = {
    ...
    endDate,
    reason: t.reason,
    source: 'winddown', // survives TeamNet sync (which only owns source:'teamnet') and links undo/adjust after reload
  };
  ...
  return logEvent(next, 'staff', `${t.label} set for ${(clinicians.find((c) => c.id === clinicianId) || {}).name || clinicianId} - wind-down ${startDate} to ${endDate}${by ? ` (by ${by})` : ''}`);
}
...
export function sweepWindDowns(data, huddleData, { getDateKeyForDay } = {}) {
  ...
  clinicians = clinicians.map((c) => {
    ...
    if (c.windDown.type === 'left' && todayIso > c.windDown.endDate) {
      ...
      dbUpdates.push({ clinicianId: c.id, fields: { status: 'left', wind_down: null } });
    ...
    if (c.windDown.type === 'sick' && huddleData && typeof getDateKeyForDay === 'function') {
      for (const day of DAYS) {
        ...
        if (csv.length && csv.some((name) => matchesStaffMember(name, c))) {
          ...
          dbUpdates.push({ clinicianId: c.id, fields: { wind_down: null } });
  ...
  if (!changed) return { changed: false, data, events: [], dbUpdates: [] };
  return { changed: true, data: { ...data, clinicians, plannedAbsences: absences }, events , dbUpdates };
}
...
export function undoTransition(data, clinicianId, { by } = {}) {
  const clinicians = (Array.isArray(data.clinicians) ? data.clinicians : Object.values(data.clinicians || {})).map((c) => {
    ...
    // Explicit null, not a dropped key: the save route only clears the
    // stored marker when the incoming clinician actually carries the field.
    return { ...c, windDown: null };
  });
  const target = (Array.isArray(data.clinicians) ? data.clinicians : []).find((c) => c.id === clinicianId);
  // Three ways to recognise the absence this wind-down created: the id it
  // was given client-side, its provenance, or simply the span it covers.
  // The span is what rescues rows written before provenance persisted.
  const wd = (Array.isArray(data.clinicians) ? data.clinicians : []).find((c) => c.id === clinicianId)?.windDown || null;
  const absences = (Array.isArray(data.plannedAbsences) ? data.plannedAbsences : [])
    .filter((a) => !(
      ...
      || (a.source === 'winddown' && a.clinicianId === clinicianId)
      || (wd && a.clinicianId === clinicianId && a.startDate === wd.startDate)
    ));
...
export function adjustTransition(data, clinicianId, newEndDate, { by } = {}) {
  ...
  const absences = (Array.isArray(data.plannedAbsences) ? data.plannedAbsences : []).map((a) =>
    ((typeof a.id === 'string' && a.id.startsWith(`winddown-${clinicianId}-`)) || (a.source === 'winddown' && a.clinicianId === clinicianId))
