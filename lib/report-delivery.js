...
import { nextSendAt, describeReportSchedule } from '@/lib/report-schedules';
...
// practice's data.
export async function sendViaResend({ to, subject, html, text, attachments, replyTo }) {
...
export async function deliverSchedule(admin, schedule, { kind = 'scheduled', overrideTo = null, siteUrl, now = new Date() } = {}) {
  ...
  try {
    ...
    const recipients = overrideTo
      ? validRecipients([{ email: overrideTo, name: '', external: false }])
      : validRecipients(schedule.recipients);

    if (recipients.length === 0) {
      log.error = overrideTo ? 'No valid address to send the test to.' : 'This schedule has no valid recipients.';
      log.status = 'skipped';
      return await finish(admin, schedule, log, kind, now);
    }
    ...
    log.status = sent.ok ? 'sent' : 'failed';
    log.provider_id = sent.providerId || null;
    log.error = sent.ok ? null : sent.error;
    return await finish(admin, schedule, log, kind, now);
  } catch (err) {
    log.status = 'failed';
