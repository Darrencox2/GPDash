...
export function getEffectivePattern(huddleData, clinician, huddleSettings = {}, opts = {}) {
  // AUTHORITY CHANGE (unification): the stored session rota IS the
  // pattern. Inference no longer decides - it annotates: uncertainty and
  // disagreement highlighting so the grid draws the eye to segments the
  // EMIS history disputes. opts.data carries the practice data for the
  // rota; without it (legacy callers) inference remains the source.
  const stats = inferPatternStats(huddleData, clinician, huddleSettings, opts);
  const rotaFor = (day) => {
    const sr = opts?.data?.sessionRota?.[clinician?.id]?.[day];
    return Array.isArray(sr) ? sr : null;
  };
  const pins = getPatternPins(huddleSettings, clinician?.id);
  const out = {};
  for (const day of DAYS) {
    const rota = rotaFor(day);
    const detail = {};
    for (const sl of SLOTS) {
      const st = stats[day][sl];
      if (rota) {
        const on = rota.includes(sl);
        const disagrees = st.obs >= 2 && st.on !== on;
        detail[sl] = { on, source: 'rota', confidence: disagrees || st.confidence === 'uncertain' ? 'uncertain' : 'confident', disagrees, ratio: st.ratio, obs: st.obs };
      } else {
        const pin = pins?.[day]?.[sl];
        if (pin === 'on' || pin === 'off') {
          detail[sl] = { on: pin === 'on', source: 'pinned', confidence: null, ratio: st.ratio, obs: st.obs };
        } else {
