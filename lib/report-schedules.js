...
// Recipients that should actually receive the next send: everyone who has
// not unsubscribed from this schedule and is not on the practice-wide
// suppression list.
//
// `suppressed` is a Set of lowercased addresses.
export function activeRecipients(recipients, suppressed = new Set()) {
  return (Array.isArray(recipients) ? recipients : [])
    .filter(r => r && r.email && !r.unsubscribedAt)
    .filter(r => !suppressed.has(String(r.email).toLowerCase()));
}

// Two links per recipient, both carrying only an opaque token:
//   page — what the human clicks; shows a confirmation before acting, so a
//          mail scanner following links cannot unsubscribe anyone
//   post — the RFC 8058 one-click endpoint named in List-Unsubscribe-Post,
//          which only ever receives POSTs from the mail client itself
export function unsubscribeUrls(siteUrl, token) {
  const base = String(siteUrl || '').replace(/\/$/, '');
  return {
    page: `${base}/r/unsubscribe/${token}`,
    post: `${base}/api/v4/public/unsubscribe/${token}`,
  };
}

// The PostgREST filter value for "the schedule holding this token".
//
// It must be JSON *text*. Handing supabase-js an array instead makes it build
// a Postgres array literal (cs.{...}), which a jsonb column rejects with
// "invalid input syntax for type json" — and because that arrives as an error
// rather than an empty match, every unsubscribe link silently reads as
// expired. Lives here, and is tested, so it does not get tidied back into an
// array by someone who reasonably assumes an array is the natural argument.
export function recipientTokenFilter(token) {
