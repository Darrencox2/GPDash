...
export function parseHuddleCSV(csvText) {
  ...
  for (let i = dataStartRowIndex + 1; i < lines.length; i++) {
    ...
    if (!isAvailable && !isBooked && !isEmbargoed) continue;
    const session = currentTime?.includes('Before') ? 'am' : 'pm';
    const targetStore = isAvailable ? dateData : isEmbargoed ? embargoedData : bookedData;
  ...
  return { clinicians, allSlotTypes: Array.from(allSlotTypes), reportDate, dates: sortedDates, dateData, bookedData, embargoedData };
}
...
export function getHuddleCapacity(parsedData, dateStr, huddleSettings, slotOverrides = null) {
  ...
  let amTotal = 0, pmTotal = 0, amEmbargoed = 0, pmEmbargoed = 0;
  const slotTypeTotals = {};

  ['am', 'pm'].forEach(session => {
    // Build a map of clinician index -> { available, embargoed } for merging
    const clinicianMap = {};
    ...
    Object.entries(dayData[session] || {}).forEach(([idx, slots]) => {
      ...
      Object.entries(slots).forEach(([slotType, count]) => {
        ...
        if (!slotTypeTotals[slotType]) slotTypeTotals[slotType] = { am: 0, pm: 0, amEmb: 0, pmEmb: 0 };
        slotTypeTotals[slotType][session] += count;
      });
      if (!clinicianMap[idx]) clinicianMap[idx] = { name: clinicianName, available: 0, embargoed: 0 };
    ...
    Object.values(clinicianMap).forEach(entry => {
      if (entry.available > 0 || entry.embargoed > 0) {
        if (session === 'am') { amByClinician.push(entry); amTotal += entry.available; amEmbargoed += entry.embargoed; }
        else { pmByClinician.push(entry); pmTotal += entry.available; pmEmbargoed += entry.embargoed; }
      }
    });
  });

  amByClinician.sort((a, b) => (b.available + b.embargoed) - (a.available + a.embargoed));
  pmByClinician.sort((a, b) => (b.available + b.embargoed) - (a.available + a.embargoed));
  const bySlotType = Object.entries(slotTypeTotals)
    .map(([name, counts]) => ({ name, am: counts.am, pm: counts.pm, amEmb: counts.amEmb, pmEmb: counts.pmEmb, total: counts.am + counts.pm, totalEmb: counts.amEmb + counts.pmEmb }))
    .filter(s => s.total > 0 || s.totalEmb > 0).sort((a, b) => (b.total + b.totalEmb) - (a.total + a.totalEmb));

  return { am: { total: amTotal, embargoed: amEmbargoed, byClinician: amByClinician }, pm: { total: pmTotal, embargoed: pmEmbargoed, byClinician: pmByClinician }, bySlotType };
}
...
export function getNDayAvailability(huddleData, huddleSettings, numDays, slotOverrides = null) {
  ...
  const results = [];
  const maxScan = numDays * 2; // account for weekends

  for (let i = 0; i < maxScan && results.length < numDays; i++) {
    ...
    if (!huddleData.dates.includes(dateStr)) {
      results.push({ date: dateStr, dayName: ['','Mon','Tue','Wed','Thu','Fri'][d.getDay()], dayNum, monthShort, available: null, embargoed: null, booked: null });
      continue;
    }
    ...
    results.push({ date: dateStr, dayName: ['','Mon','Tue','Wed','Thu','Fri'][d.getDay()], dayNum, monthShort, available: cap.am.total + cap.pm.total, embargoed: (cap.am.embargoed || 0) + (cap.pm.embargoed || 0), booked: totals.booked });
  }
  return results;
}
