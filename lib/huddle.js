...
export function parseHuddleCSV(csvText) {
  ...
  return { clinicians, allSlotTypes: Array.from(allSlotTypes), reportDate, dates: sortedDates, dateData, bookedData, embargoedData, locationData };
}
...
export function getDutyDoctor(parsedData, dateStr, session, dutySlotTypes) {
  ...
  const stores = [parsedData.dateData, parsedData.bookedData, parsedData.embargoedData];
  const clinicians = parsedData.clinicians;
