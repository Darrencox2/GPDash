...
export function mergeHuddleData(existing, incoming) {
  ...
  // Merge clinicians and slot types (union)
  ...
  // Merge date-keyed stores
  const mergeStore = (oldStore, newStore) => {
    const merged = {};
    // Keep locked old dates
    Object.keys(oldStore || {}).forEach(dateStr => {
      const d = parseHuddleDateStr(dateStr);
      if (d < pruneCutoff) return; // Prune
      if (d < lockCutoff) merged[dateStr] = oldStore[dateStr]; // Lock
      // Dates within 3 days: new data wins (handled below)
    });
    // Add/overwrite from new data (all dates in new CSV)
    Object.keys(newStore || {}).forEach(dateStr => {
      const d = parseHuddleDateStr(dateStr);
      if (d < pruneCutoff) return; // Prune
      if (d < lockCutoff && merged[dateStr]) return; // Don't overwrite locked
      merged[dateStr] = newStore[dateStr];
    });
    return merged;
  };

  const dateData = mergeStore(existing.dateData, incoming.dateData);
  const bookedData = mergeStore(existing.bookedData, incoming.bookedData);
  const embargoedData = mergeStore(existing.embargoedData, incoming.embargoedData);
  const blockedData = mergeStore(existing.blockedData, incoming.blockedData);
  const locationData = mergeStore(existing.locationData, incoming.locationData);
  const splitSiteData = mergeStore(existing.splitSiteData, incoming.splitSiteData);
  const slotLocationData = mergeStore(existing.slotLocationData, incoming.slotLocationData);
  // slotRows uses the same merge rules — locked old, prune ancient, new wins.
  // Older parsed CSVs predate slotRows, so existing.slotRows may be missing
  // entirely; mergeStore handles undefined cleanly.
  const slotRows = mergeStore(existing.slotRows, incoming.slotRows);
