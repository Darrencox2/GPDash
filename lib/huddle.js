...
export function parseHuddleCSV(csvText) {
  ...
  const blockedData = {};
  const locationData = {};
  const clinSessionLocData = {}; // { dateStr: { session: { clinicianIdx: { location: count } } } }
    ...
    for (let j = clinicianStartIdx; j < cells.length && (j - clinicianStartIdx) < clinicians.length; j++) {
      ...
      if (count > 0) {
        ...
        if (location) {
          ...
          // Per-session per-clinician location
          if (!clinSessionLocData[currentDate]) clinSessionLocData[currentDate] = {};
          if (!clinSessionLocData[currentDate][session]) clinSessionLocData[currentDate][session] = {};
          if (!clinSessionLocData[currentDate][session][idx]) clinSessionLocData[currentDate][session][idx] = {};
          clinSessionLocData[currentDate][session][idx][location] = (clinSessionLocData[currentDate][session][idx][location] || 0) + count;
  ...
  return { clinicians, allSlotTypes: Array.from(allSlotTypes), reportDate, dates: sortedDates, dateData, bookedData, embargoedData, blockedData, locationData, clinSessionLocData, slotLocationData };
}
...
export function getClinicianSessionLocations(parsedData, dateStr) {
  if (!parsedData?.clinicians || !parsedData?.clinSessionLocData?.[dateStr]) return {};
  const result = {};
  const sessData = parsedData.clinSessionLocData[dateStr];
  const clinicians = parsedData.clinicians;
  const allIndices = new Set();
  ['am', 'pm'].forEach(sess => { if (sessData[sess]) Object.keys(sessData[sess]).forEach(idx => allIndices.add(parseInt(idx))); });
  allIndices.forEach(idx => {
    const name = clinicians[idx];
    if (!name) return;
    const entry = { am: null, pm: null };
    ['am', 'pm'].forEach(sess => {
      const locMap = sessData[sess]?.[idx];
      if (!locMap) return;
      let maxLoc = null, maxCount = 0;
      Object.entries(locMap).forEach(([loc, count]) => { if (count > maxCount) { maxLoc = loc; maxCount = count; } });
      entry[sess] = maxLoc ? (LOCATION_SHORTNAMES[maxLoc] || maxLoc) : null;
    });
    result[name] = entry;
  });
  return result;
