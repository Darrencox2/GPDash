...
export function getHuddleCapacity(parsedData, dateStr, huddleSettings, slotOverrides = null) {
  ...
  const isSlotIncluded = (slotType) => {
    if (slotOverrides) return slotOverrides[slotType] !== undefined ? slotOverrides[slotType] : true;
  ...
  ['am', 'pm'].forEach(session => {
    const clinicianMap = {};
    const initEntry = (idx, name) => { if (!clinicianMap[idx]) clinicianMap[idx] = { name, available: 0, embargoed: 0, booked: 0 }; };
