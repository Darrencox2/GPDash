...
export function getNDayAvailability(huddleData, huddleSettings, numDays, slotOverrides = null) {
  ...
  for (let i = 0; i < maxScan && results.length < numDays; i++) {
    ...
    if (!huddleData.dates.includes(dateStr)) {
      results.push({ date: dateStr, dayName: ['','Mon','Tue','Wed','Thu','Fri'][d.getDay()], dayNum, monthShort, available: null, booked: null });
      continue;
    }
    ...
    results.push({ date: dateStr, dayName: ['','Mon','Tue','Wed','Thu','Fri'][d.getDay()], dayNum, monthShort, available: cap.am.total + cap.pm.total, booked: totals.booked });
  }
  return results;
}
