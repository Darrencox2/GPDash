...
export function parseHuddleCSV(csvText) {
  ...
  return { clinicians, allSlotTypes: Array.from(allSlotTypes), reportDate, dates: sortedDates, dateData, bookedData, embargoedData, blockedData, locationData, slotLocationData };
}
...
// Derives from slot data + location data without needing clinSessionLocData stored
export function getClinicianSessionLocations(parsedData, dateStr) {
  ...
  // Find all clinician indices active on this date
  [parsedData.dateData, parsedData.bookedData, parsedData.embargoedData].forEach(store => {
    if (!store?.[dateStr]) return;
    ['am', 'pm'].forEach(sess => {
  ...
  // For each clinician, determine per-session location
  // Use slotLocationData to correlate slot types with locations per session
  allIndices.forEach(idx => {
    const name = clinicians[idx];
    if (!name) return;
    const entry = { am: null, pm: null };

    ['am', 'pm'].forEach(sess => {
      // Check if clinician has any slots in this session
      let hasSlots = false;
      [parsedData.dateData, parsedData.bookedData, parsedData.embargoedData].forEach(store => {
        if (store?.[dateStr]?.[sess]?.[idx]) hasSlots = true;
      });
      if (!hasSlots) return;

      // Find location for this clinician in this session
      // Cross-reference their slot types with slotLocationData
      const locCounts = {};
      [parsedData.dateData, parsedData.bookedData, parsedData.embargoedData].forEach(store => {
        const clinSlots = store?.[dateStr]?.[sess]?.[idx];
        if (!clinSlots) return;
        Object.keys(clinSlots).forEach(slotType => {
          const slotLocs = parsedData.slotLocationData?.[dateStr]?.[sess]?.[slotType];
          if (slotLocs) {
            Object.entries(slotLocs).forEach(([loc, count]) => {
              locCounts[loc] = (locCounts[loc] || 0) + count;
            });
          }
        });
      });

      // If we got location data from slot correlation, use it
      if (Object.keys(locCounts).length > 0) {
        const top = Object.entries(locCounts).sort((a, b) => b[1] - a[1])[0][0];
        entry[sess] = top; // Already shortened via LOCATION_SHORTNAMES in slotLocationData
      } else {
        // Fallback to overall locationData
        const locMap = parsedData.locationData?.[dateStr]?.[idx];
        if (locMap) {
          const top = Object.entries(locMap).sort((a, b) => b[1] - a[1])[0][0];
          entry[sess] = LOCATION_SHORTNAMES[top] || top;
        }
      }
    });
