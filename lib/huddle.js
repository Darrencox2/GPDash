...
export function parseHuddleCSV(csvText) {
  ...
  return { clinicians, allSlotTypes: Array.from(allSlotTypes), reportDate, dates: sortedDates, dateData, bookedData, embargoedData, blockedData, locationData, splitSiteData, slotLocationData };
}
...
export function mergeHuddleData(existing, incoming) {
  ...
  // ─── Index re-keying ───────────────────────────────────────────────
  // BUG until v3.4.1: per-date slot data was stored keyed by CSV column
  ...
  // (ex-locums no longer at the practice) appeared to "have appointments"
  // that actually belonged to someone else.
  //
  ...
  // Re-key a flat { idx: value } map (used for locationData, splitSiteData).
  ...
  return { clinicians, allSlotTypes, reportDate: incoming.reportDate, dates, dateData, bookedData, embargoedData, blockedData, locationData, splitSiteData, slotLocationData };
}
