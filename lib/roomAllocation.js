...
// Curated palette designed for the dark-glass UI. Avoids the status
// colours used for capacity bands (red = short, amber = tight, emerald
// = good) so site indicators don't read as health states. Ordered for
// good contrast between adjacent picks — the first three hues are
// well-separated so a typical 1–3 site practice gets visually distinct
// markers without effort.
export const SITE_COLOUR_PRESETS = [
  '#8b5cf6', // violet — first default
  '#06b6d4', // cyan
  '#f97316', // orange
  '#ec4899', // pink
  '#84cc16', // lime
...
export function getRoomTypesForClinician(clinician) {
  // Latent rather than live — the one caller guards with `if (!clin) return`
  // — but every comparable helper in this codebase tolerates a missing
  // record, and a room dashboard is not worth crashing over a stale id.
  if (!clinician) return ['admin'];
...
export function autoAllocateRooms(site, session, cliniciansAtSite, recurringBookings, adHocBookings, dateStr, allClinicians, priorityOrder, dailyOverrides, procedureFlags) {
  if (!site || !site.rooms) return { assignments: {}, conflicts: [], flags: [] };
  const clinicalRooms = site.rooms.filter(r => r.isClinical !== false);
  const assignments = {};
  const flags = [];
  const conflicts = [];
  const procFlags = procedureFlags || {};
  ...
  cliniciansAtSite.forEach(c => {
    ...
    if (!clin) return;
    needsRoom.push({ id: c.id, name: clin.name, initials: clin.initials, types: procFlags[c.id] ? ['nurse_procedure'] : getRoomTypesForClinician(clin), preferredRoom: clin.roomPreferences?.[site.id]?.preferred, secondaryRoom: clin.roomPreferences?.[site.id]?.secondary, source: 'csv', priority: priorityOrder.indexOf(c.id), needsProcedure: !!procFlags[c.id] });
  });
