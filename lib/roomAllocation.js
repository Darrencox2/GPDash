...
export const GRID_SIZES = {
  small: { cols: 6, rows: 5, label: 'Small (up to 15 rooms)' },
  medium: { cols: 8, rows: 6, label: 'Medium (up to 25 rooms)' },
  large: { cols: 10, rows: 8, label: 'Large (up to 40 rooms)' },
};
...
export function autoAllocateRooms(site, session, cliniciansAtSite, recurringBookings, adHocBookings, dateStr, allClinicians, priorityOrder, dailyOverrides) {
  ...
  if (override) {
    return { assignments: override, conflicts: [], flags: Object.entries(override).filter(([_, v]) => v?.isOverride).map(([_, v]) => ({ name: v.name, message: 'Manually assigned' })) };
  }
  ...
  needsRoom.filter(p => !p.assigned).forEach(person => {
    if (person.secondaryRoom && !usedRooms.has(person.secondaryRoom)) {
      ...
      if (room) {
        ...
        usedRooms.add(room.id);
        person.assigned = true;
        flags.push({ name: person.name, message: `Not in preferred room — using secondary (${room.name})` });
      }
    }
  });
  ...
  needsRoom.filter(p => !p.assigned).forEach(person => {
    ...
    if (available.length > 0) {
      ...
      usedRooms.add(room.id);
      person.assigned = true;
      flags.push({ name: person.name, message: `Not in preferred room — assigned to ${room.name}` });
    }
  });
