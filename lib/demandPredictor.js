...
export function predictDemand(inputDate, weather, options) {
  ...
  // Resolve effective coefficients — practice settings if present, else defaults
  var BASELINE_eff = (ds && typeof ds.baseline === 'number') ? ds.baseline : BASELINE;
  var DOW_eff = (ds && Array.isArray(ds.dowEffects) && ds.dowEffects.length === 5) ? ds.dowEffects : DOW_EFFECTS;
  var MONTH_eff = (ds && Array.isArray(ds.monthEffects) && ds.monthEffects.length === 12) ? ds.monthEffects : MONTH_EFFECTS;
