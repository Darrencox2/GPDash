...
// Build a fresh school-holiday set (and derived first-week-back set) from
// an array of holiday ranges. Accepts either:
//   - [{start: 'YYYY-MM-DD', end: 'YYYY-MM-DD'}, ...] (object form)
//   - [['YYYY-MM-DD', 'YYYY-MM-DD'], ...]             (tuple form, as used
//     by lib/school-holidays-by-lea.js)
function buildHolidayContext(ranges) {
  if (!Array.isArray(ranges) || ranges.length === 0) return null;
  const set = new Set();
  for (const r of ranges) {
    let start, end;
    if (Array.isArray(r) && r.length >= 2) { start = r[0]; end = r[1]; }
    else if (r?.start && r?.end) { start = r.start; end = r.end; }
    else continue;
  ...
  const fwb = buildFirstWeekBackSet(set);
  return { schoolHolSet: set, firstWeekBackSet: fwb };
}

/**
 * Predict daily demand for a date.
 *
 * @param {Date|string} inputDate
 * @param {object|null} weather  - { temp, feelsLike, precipMm, yesterdayPrecipMm } or null
 * @param {object} [options]
 *   @prop {object} demandSettings  - per-practice calibration:
 *     { baseline, dowEffects, monthEffects, growthPerDay, referenceDate }
 *     When provided, replaces the default Winscombe-calibrated constants.
 *   @prop {Array<{start,end}>} schoolHolidayRanges  - per-LEA holiday
 *     ranges. When provided, replaces the default North Somerset set.
 *   @prop {number} baselineAdjustment  - per-practice recalibration offset
 *     (overrides the module-level _baselineAdjustment global).
 *
 * Backward-compatible: with no `options`, behaves the same as a call with
 * `options.listSize = 11000` — i.e. produces the original Winscombe-shaped
 * numbers. Pass `listSize` to scale the fallback for a different practice
 * size, or pass `demandSettings` for a fully calibrated prediction.
 */
export function predictDemand(inputDate, weather, options) {
  weather = weather || null;
  options = options || {};
  var ds = options.demandSettings || null;

  // ─── Fallback scaling for practices with no demand_settings yet ───
  // The hardcoded BASELINE/DOW/MONTH constants were calibrated against an
  // 11k-list practice (Winscombe). For a practice that hasn't yet had
  // its NHS auto-seed run or a CSV uploaded, we get a much better
  // first-time estimate by scaling those constants in proportion to the
  // practice's list size: a 5,500-patient practice gets half the
  // Winscombe baseline, an 18,000-patient practice gets ~1.6× — which
  // is roughly what you'd expect from per-1000 averages across the UK.
  // The 'usingFallback' flag on the result lets callers show a warning
  // banner ("estimated — upload data for accurate prediction").
  var REFERENCE_LIST_SIZE = 11000;
  var listSize = typeof options.listSize === 'number' && options.listSize > 0
    ? options.listSize
    : null;
  var usingFallback = !ds;
  var fallbackScale = (usingFallback && listSize)
    ? listSize / REFERENCE_LIST_SIZE
    : 1;

  // Resolve effective coefficients — practice settings if present, else
  // list-size-scaled fallback (or unscaled if no list size known)
  var BASELINE_eff = (ds && typeof ds.baseline === 'number')
    ? ds.baseline
    : BASELINE * fallbackScale;
  var DOW_eff = (ds && Array.isArray(ds.dowEffects) && ds.dowEffects.length === 5)
    ? ds.dowEffects
    : (usingFallback && fallbackScale !== 1
        ? DOW_EFFECTS.map(function (v) { return v * fallbackScale; })
        : DOW_EFFECTS);
  var MONTH_eff = (ds && Array.isArray(ds.monthEffects) && ds.monthEffects.length === 12)
    ? ds.monthEffects
    : (usingFallback && fallbackScale !== 1
        ? MONTH_EFFECTS.map(function (v) { return v * fallbackScale; })
        : MONTH_EFFECTS);
  var growthPerDay = (ds && typeof ds.growthPerDay === 'number') ? ds.growthPerDay : null;
  var refDate = (ds && ds.referenceDate) ? new Date(ds.referenceDate + 'T00:00:00') : REFERENCE_DATE;

  // Resolve effective school-holiday context — custom LEA if provided
  var custom = options.schoolHolidayRanges
    ? buildHolidayContext(options.schoolHolidayRanges)
    : null;
  var schoolHolSet_eff = custom ? custom.schoolHolSet : schoolHolSet;
  var firstWeekBackSet_eff = custom ? custom.firstWeekBackSet : firstWeekBackSet;

  // Per-practice baseline adjustment (otherwise fall through to the global
  // module-level value used by recalibrateBaseline()).
  var baselineAdj = (typeof options.baselineAdjustment === 'number')
  ...
  var factors = {};
  var pred = BASELINE_eff;
  factors.baseline = BASELINE_eff;

  // 1. Day of week
  pred += DOW_eff[dow];
  factors.dayOfWeek = { day: DOW_NAMES[dow], effect: DOW_eff[dow] };

  // 2. Month
  pred += MONTH_eff[month];
  factors.month = { month: month + 1, effect: MONTH_eff[month] };

  // 3. School holiday
  var isSchoolHol = schoolHolSet_eff.has(dk);
  if (isSchoolHol) { pred += SCHOOL_HOL_EFFECT; factors.schoolHoliday = SCHOOL_HOL_EFFECT; }

  // 4. First week back after school holiday
  var isFWB = firstWeekBackSet_eff.has(dk);
  ...
  // 9. Growth trend — per-practice if demandSettings supplied a growthPerDay,
  // otherwise the legacy MONTHLY_TREND constant applied month-by-month.
  //
  // Safety clamp: trend effect can't exceed ±25% of baseline regardless of
  // what the calibration produced. Protects against wild slopes from short
  // calibration windows (the recalibration code now requires 90 days of
  // history before computing a slope, but practices that calibrated
  // earlier may have settings with too-large growthPerDay values stored).
  // The user sees `clamped: true` in factors.trend if the cap kicked in.
  if (growthPerDay !== null) {
    var daysFromRef = Math.round((d - refDate) / 86400000);
    var rawTrend = growthPerDay * daysFromRef;
    var maxTrend = Math.abs(BASELINE_eff) * 0.25;
    var growthEff = rawTrend;
    var clamped = false;
    if (growthEff > maxTrend) { growthEff = maxTrend; clamped = true; }
    if (growthEff < -maxTrend) { growthEff = -maxTrend; clamped = true; }
    pred += growthEff;
    factors.trend = {
      daysFromRef: daysFromRef,
      perDay: growthPerDay,
      effect: Math.round(growthEff * 10) / 10,
      rawEffect: Math.round(rawTrend * 10) / 10,
      clamped: clamped,
    };
  } else {
    var monthsElapsed = monthsBetween(refDate, d);
  ...
  // 12. Baseline recalibration (per-practice if supplied, else module global)
  pred += baselineAdj;
  if (baselineAdj !== 0) { factors.recalibration = baselineAdj; }
  ...
  return {
    ...
    // True when no per-practice demand_settings was passed in. UI should
    // surface a banner so users know the prediction is an estimate.
    usingFallback: usingFallback,
    fallbackScale: usingFallback ? fallbackScale : null,
  };
}
...
// getWeatherForecast — Open-Meteo daily forecast for the practice location.
// Backward-compatible: with no lat/lon args, defaults to Winscombe (51.32, -2.84).
// Practice-specific callers pass coords from practices.latitude/longitude
// (set during postcode-lookup at setup time).
...
function classifyDemand(predicted) {
...
function getStaffingRecommendation(predicted) {
