...
export function computeMonthlySpend({ huddleData, data, month }) {
  ...
  for (const lc of locums) {
    ...
    for (const { ds, d } of monthDates) {
      ...
      if (!n) continue;
      sessions += n;
      total += n * rateForDate(locumRates[lc.id], isoOf(d));
    }
    if (!sessions) continue;
    const rate = sessions ? total / sessions : 0;
    locumLines.push({ id: lc.id, name: lc.name, sessions, rate, total, rateMissing: !total });
  }
