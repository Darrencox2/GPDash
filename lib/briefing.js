...
const REASON_LABELS = {
  ...
  long_term_absence: 'Long-term sick', 'leaving_-_wind_down_cover': 'Leaving (working notice)',
};
...
export function assembleBriefing({ data, huddleData, huddleMessages, date = new Date(), predictionOptions = null }) {
  ...
  const reasonOf = (id) => {
    ...
    const wd = c?.windDown;
    if (wd && dateKey >= wd.startDate && dateKey <= wd.endDate) return wd.type === 'sick' ? 'Long-term sick' : 'Working notice';
    return 'Absent';
  };
