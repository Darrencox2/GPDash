// Older releases, newest first. Loaded on demand by the changelog views;
// not part of the main bundle. See lib/changelog.js.
export const CHANGELOG_ARCHIVE = [
  {
    version: '4.131.0',
    date: '2026-08-31',
    title: 'Capacity planning, refined',
    changes: [
      { type: 'fix', text: 'A bank holiday week is no longer judged against a full week target. The weekly routine target now scales by the days the practice is actually open, so a four day week aims at 200 rather than 250, says short week beside the number, and stops reporting a guaranteed miss every single bank holiday' },
      { type: 'fix', text: 'Far future weeks no longer scream red about bookings that have not opened. More than three weeks out, with essentially nothing booked yet, a short session shows as a quiet dashed outline instead of solid red, so red only appears where action is possible. The key explains the new marking and the hover says beyond the booking horizon' },
      { type: 'tweak', text: 'In the summary strip, urgent below target now clearly leads in red with an arrow, and the four exploration cards sit quiet and neutral until opened. Five equally loud coloured cards used to compete' },
      { type: 'tweak', text: 'The colour key has moved above the grid it explains. It was the last line of the card, after all sixty tiles' },
      { type: 'tweak', text: 'The urgent slots heading is written once across the top instead of five times in red capitals, one per day column, and the day names step up a size' },
      { type: 'tweak', text: 'The bank holiday tile now uses the same quiet house icon treatment as every other closed day in the app, instead of an amber box in the colour that elsewhere means tight' },
      { type: 'feature', text: 'Clicking a week row opens that week in the day by day detail view. The detail view existed but the obvious thing to click, the week itself, did nothing' },
      { type: 'tweak', text: 'The weekly routine bars are down to three marks, offered, booked and the target tick. The comfort band detail moved into the hover text where it can be read as words' },
      { type: 'tweak', text: 'The site staffing badge now says 29 short rather than a bare 29' },
      { type: 'tweak', text: 'The clinician search no longer shows a large empty illustration before anyone has typed, just a single quiet line under the search box' },
    ]
  },
  {
    version: '4.130.0',
    date: '2026-08-31',
    title: 'Buddy cover board, refined',
    changes: [
      { type: 'tweak', text: 'Each row on the week board now says covers between the person who is in and the people they are covering, and the key explains that bold initials mean in today. The tiny arrow it replaces was six pixels wide and explained nothing' },
      { type: 'tweak', text: 'A closed day collapses to a slim labelled rail instead of taking a full column to say one word, so the four real days get the space. Clicking the rail still opens the closed day, where it can be marked open' },
      { type: 'tweak', text: 'When the board matches EMIS, the checking panel is a single quiet line instead of a full column saying nothing is wrong. The full panel returns the moment there is something to fix' },
      { type: 'tweak', text: 'When every day of the week is generated, one line in the board header says all four days ready instead of four identical pills. The pills return the moment any day differs' },
      { type: 'tweak', text: 'The leave row says how many are on leave up front, and the plus number now opens to show everyone, with names and reasons on hover. It used to hide most of the list behind a number that did nothing' },
      { type: 'tweak', text: 'The week arrows now sit either side of the date they change instead of at opposite corners of the board, with the jump back to this week beside them' },
      { type: 'fix', text: 'If the TeamNet sync is more than a day old the header now says so in amber with the age and a sync now link. The board is generated from that sync, and a stale one used to look identical to a fresh one' },
      { type: 'tweak', text: 'The selected day and the detail below it are now visibly joined, the purple underline on the chosen day continues as the top edge of its detail card, like a tab and its panel' },
      { type: 'tweak', text: 'Copy Week is now a quiet outline so Generate 4 Weeks, the weekly action, clearly leads' },
    ]
  },
  {
    version: '4.129.3',
    date: '2026-08-31',
    title: 'A false alarm in the console silenced',
    changes: [
      { type: 'fix', text: 'The console error claiming the slim huddle endpoint returned empty was itself wrong. The check could not tell an empty response from a cancelled one, and in development React deliberately runs every effect twice, so the first cancelled run was misread as an empty endpoint on every single dashboard load. It then fetched the full practice payload again for nothing. The endpoint was never at fault. The check now tells the two cases apart, and the heavy fallback only fires when the endpoint genuinely returns nothing' },
    ]
  },
  {
    version: '4.129.2',
    date: '2026-08-31',
    title: 'Sidebar footer tidied',
    changes: [
      { type: 'tweak', text: 'The bottom of the sidebar is one compact block instead of four stacked sections. The light mode toggle and the text size control now sit side by side, and the version number and collapse arrow share a single quiet line beneath them. Nothing was removed' },
      { type: 'fix', text: 'On a typical laptop screen the navigation no longer scrolls. The old footer was tall enough to squeeze the menu into scrolling, so reaching the lower sections meant scrolling past nothing' },
    ]
  },
  {
    version: '4.129.1',
    date: '2026-08-31',
    title: 'Duty doctor row tinted in the site colour',
    changes: [
      { type: 'tweak', text: 'The duty doctor row now has a soft fill in the colour of the site they are covering, as in the approved design, rather than a plain neutral card. At Winscombe that is the purple' },
    ]
  },
  {
    version: '4.129.0',
    date: '2026-08-31',
    title: 'The Today page, refined',
    changes: [
      { type: 'tweak', text: 'The duty doctor row is no longer a solid red alarm. It keeps its place at the top with the star, but is now styled as the important role it is, with the site colour as an edge stripe. Red stays reserved for things that are actually wrong' },
      { type: 'tweak', text: 'The duty doctor no longer shows a zero. Duty triages rather than holding bookable urgent slots, so the row now says triage instead of a number that looked like a fault' },
      { type: 'fix', text: 'The NHS benchmark strip has stopped telling you to set a list size that was already set. The real cause was that NHS Digital publishes a month before its figures arrive, and the strip was reading that empty month. It now reads the newest month with actual data, so it shows your PCN and national comparison instead of a wrong warning' },
      { type: 'tweak', text: 'The noticeboard collapses to a single line when it has nothing on it, which is most days, and expands the moment a notice is posted' },
      { type: 'tweak', text: 'The gauge and the tile next to it no longer repeat the same number. The gauge keeps the percentage against target, the tile keeps the count' },
      { type: 'tweak', text: 'The rule for reading the whos in badges, left is morning, right is afternoon, has moved from the bottom of the card to the top, where you need it' },
      { type: 'feature', text: 'Routine capacity now shows the previous four weeks alongside the forward view, what each week offered and how full it ended, so the forward numbers finally have context' },
      { type: 'fix', text: 'The date markers under the long capacity charts were drawn in a colour so faint they were invisible, which made a 28 day chart read as having no dates at all. Mondays now show a readable date' },
      { type: 'tweak', text: 'The demand insight is one line instead of a card, with the full factor breakdown still one click away' },
    ]
  },
  {
    version: '4.128.0',
    date: '2026-08-30',
    title: 'Errors you can copy, and no more failed to load chunk',
    changes: [
      { type: 'fix', text: 'The failed to load chunk message is gone. It happens when a tab is left open while a new version goes out, so the browser asks for a file that no longer exists. The app is not broken in that situation, it is simply out of date, so it now reloads itself once and carries on. If it cannot, it says the app has been updated and offers a reload button, rather than showing a raw error' },
      { type: 'feature', text: 'Error boxes now have a copy button. What it copies is a full report, the message, the version, which screen you were on, which practice, your browser and screen size, and a numbered list of the last few screens you visited before it happened. Pasting that into a message is far more useful than a screenshot, because it says what led to the problem' },
      { type: 'tweak', text: 'The error box also shows the last few things you did on screen, so you can see the trail without copying anything' },
      { type: 'note', text: 'Genuine crashes are still treated as crashes. Only the out of date file case reloads, and only once per visit, so a real fault can never hide behind an endless refresh' },
    ]
  },
  {
    version: '4.127.0',
    date: '2026-08-30',
    title: 'Choose your own text size',
    changes: [
      { type: 'feature', text: 'A text size control now sits under the light mode toggle, with three settings, compact, default and large. It is remembered in your own browser, so one person can run the app larger without changing it for anybody else at the practice' },
      { type: 'note', text: 'Default is exactly what the app has always looked like, so nothing changes for anyone unless they choose it. Compact removes the built in magnification and is the size a dashboard like this normally runs at. Large goes further than the current default for anyone who wants it' },
      { type: 'note', text: 'The steps are gentler on smaller screens, where a heavy magnification would push the layout sideways, and no setting ever goes below normal size, so nothing can shrink under the readable floor' },
      { type: 'tweak', text: 'When the sidebar is collapsed the control becomes a single button that cycles through the three sizes, since three would not fit' },
    ]
  },
  {
    version: '4.126.0',
    date: '2026-08-30',
    title: 'Framework upgraded to Next 16, and the last known vulnerability is gone',
    changes: [
      { type: 'feature', text: 'Upgraded to the next major version of the web framework. The security audit now reports zero vulnerabilities, down from four earlier today. The last one could only be cleared by this upgrade' },
      { type: 'note', text: 'It went through in one attempt because the groundwork was already done. Seven of the nine breaking changes did not apply to this codebase, and the one that did, awaiting cookies before reading them, was fixed earlier today for an unrelated reason' },
      { type: 'tweak', text: 'The routing file has been renamed from middleware to proxy, as the framework now requires. Every behaviour it controls was checked afterwards, the redirect a visitor follows to the login page, the shortcut that takes a signed in user straight to their practice, the cookie that remembers which practice that is, and staying signed in across a reload' },
      { type: 'fix', text: 'A test that checks the development only security relaxation never reaches the live site was quietly checking the wrong thing after the upgrade, because the file it used to detect a development server no longer exists. It now checks two independent signals, and was confirmed to fail correctly when pointed at the wrong kind of server' },
      { type: 'fix', text: 'The menu button on phones and tablets had no name, so a screen reader announced it only as button. It is the only way to open navigation on a small screen' },
      { type: 'note', text: 'Every screen was opened at desktop, tablet and phone size after the upgrade. Ten of ten worked at all three, with nothing broken, no sideways scrolling and no unreadable text' },
    ]
  },
  {
    version: '4.125.0',
    date: '2026-08-30',
    title: 'Colour means one thing on the capacity page, and everything works from the keyboard',
    changes: [
      { type: 'tweak', text: 'Red, amber and green on the capacity page now mean capacity and nothing else. The demand figure has become a plain chip with an arrow showing how it compares to a typical day, and the weekly routine bar is a single colour with the plus or minus number carrying the judgement. Previously all three used the same three colours for three unrelated things, so a red tile, a red chip and a red bar meant entirely different things on one screen' },
      { type: 'feature', text: 'Everything clickable can now be operated from the keyboard. Forty elements that looked and behaved like buttons could only be reached with a mouse, including cards, chips and grid cells across the workforce planner, the report builder and the room screens. They are now reachable by tab and activated by enter or space' },
      { type: 'note', text: 'Dialog backdrops were deliberately left alone. Clicking outside a dialog to dismiss it is a mouse convenience, and making it a keyboard stop would put a meaningless item in the tab order' },
    ]
  },
  {
    version: '4.124.0',
    date: '2026-08-30',
    title: 'Slot types named OTD are recognised again, plus an auth library upgrade',
    changes: [
      { type: 'fix', text: 'A slot type named OTD was never suggested as urgent during setup. The rule looked for ONTD, which is not an abbreviation anyone uses, while OTD is the standard shorthand for on the day. Any practice whose data uses that name would have had those appointments left out of the urgent model unless someone noticed and set them by hand. Found by writing the first tests for this logic' },
      { type: 'feature', text: 'The slot classification rules have moved out of the setup wizard into their own file with twenty two tests. They were buried in a 3700 line screen and could not be tested at all, despite deciding how a new practice appointment data is interpreted from day one' },
      { type: 'fix', text: 'The library that manages sign in cookies has been brought up to date, seven releases on from where it was. Sign in, staying signed in across a reload, and authenticated data requests were all checked end to end afterwards' },
      { type: 'tweak', text: 'Every screen now announces a proper page heading to screen readers, and the two unlabelled date arrows on the today screen now have names. Previously two screens had no heading at all' },
    ]
  },
  {
    version: '4.123.0',
    date: '2026-08-30',
    title: 'Rate limiting reports itself, 35 more tests, and a Sunday is no longer a dead end',
    changes: [
      { type: 'feature', text: 'If the request throttle is not switched on, the errors screen now says so. It was designed to let requests through when its store is unavailable, which is the right call, but it only whispered into a log. That is exactly how the blocked weather service stayed hidden for months' },
      { type: 'feature', text: 'Thirty five new tests covering five parts of the system that had none at all, the room booking rules, buddy cover regeneration, the workload report vocabulary, the rota inference and the capacity pattern detector. Ninety two tests now run in under half a second' },
      { type: 'fix', text: 'Room type lookup no longer falls over when handed a clinician record that is not there. The one caller already guards against it so nothing was breaking, but every comparable helper in the app tolerates a missing record and this one did not' },
      { type: 'tweak', text: 'The upload button is no longer bright red. Red means delete everywhere else in the app, and this is a routine daily action that was the loudest thing on the screen. It is amber when an upload is due and green once done' },
      { type: 'tweak', text: 'On a closed day there is now a button to jump to the next open one. Landing on a Sunday used to give a large empty card and no way onward' },
      { type: 'tweak', text: 'The huddle board button is now a quieter outline. Two solid buttons side by side meant neither one led' },
      { type: 'tweak', text: 'Replaced a stray emoji on the buddy board with the same drawn icon the equivalent screen uses one click away' },
      { type: 'note', text: 'The app has now been measured on tablet and phone widths for the first time, where the page scale drops. No text under 11 pixels, no sideways scrolling, no contrast failures and nothing broken' },
    ]
  },
  {
    version: '4.122.1',
    date: '2026-08-30',
    title: 'Rota day edits actually save now, and an override write bug fixed',
    changes: [
      { type: 'fix', text: 'Changing which days someone works has not been saving. Three separate controls were affected, the day toggle on the dashboard, the day pills on a team member card, and the generated rota from the auto rota tool. All three appeared to work and silently did nothing. The server had a guard that skipped the day level save whenever session level data was also present, and since every save sends the whole data object that guard was always on. The two paths now share the work per clinician instead of one blocking the other' },
      { type: 'fix', text: 'The new daily override writer queued its deletes and its inserts together rather than in order, so a delete could land after the insert and empty the table. Caught in testing before any real use. Nothing was lost because the previous storage was deliberately kept alongside, and the table has been rebuilt from it. Writes are now strictly ordered and a test pins that ordering' },
      { type: 'note', text: 'The plan to retire that older copy this release has been dropped. It is the reason this cost nothing, so it stays until the new writer has a full release of real use behind it' },
    ]
  },
  {
    version: '4.122.0',
    date: '2026-08-30',
    title: 'Daily overrides finally live in the table built for them',
    changes: [
      { type: 'feature', text: 'The daily_overrides table was created in May and never written to, not one row. Every in and out change has instead been stored as a single blob of text on the practice settings row, in the old v3 format. That blob is now copied into the real table, which is where the rest of the v4 data model already lives' },
      { type: 'feature', text: 'Because the table stores a state per clinician per session rather than one lump per day, a future change can mark someone off for just the morning without another database migration. Todays day level data fills both sessions, so nothing changes today' },
      { type: 'note', text: 'Nothing about the huddle board changes. The app still reads the same shape it always did, so all 45 places that use this data were left untouched. The only difference is where it is stored. Checked against all 22 days of real data: the set of people shown as absent is identical before and after' },
      { type: 'note', text: 'Saves write to both places for this release. The table is now the record, the old blob is kept in step as a fallback, and reads use the blob for any practice not yet copied across. Once the table is proven the blob writing can be removed' },
      { type: 'feature', text: 'Fifteen new tests cover the conversion in both directions, including the case where someone turns up who was not scheduled' },
    ]
  },
  {
    version: '4.121.0',
    date: '2026-08-30',
    title: 'Weather unblocked, readable text everywhere, a key on the buddy board',
    changes: [
      { type: 'fix', text: 'The weather forecast has been blocked by our own security header since it was added. The demand prediction calls Open-Meteo, the header listed every allowed outside address except that one, and the browser refused the request on every single page load. Demand has therefore been predicted without any weather input, while the published list of services we use named Open-Meteo as a supplier. The address is now allowed and the forecast loads' },
      { type: 'feature', text: 'Security header violations are now recorded on the errors screen instead of a log nobody reads. The browser has been dutifully reporting the blocked weather requests to us the whole time and the report handler simply wrote them to the console. That is how a broken feature stayed invisible for months' },
      { type: 'fix', text: 'Text across the signed in app raised to meet contrast standards. There were 209 failures, now 43, and most of what remains sits a hundredth under the threshold. The main causes were a very faint grey used for small print and white text on bright fills' },
      { type: 'fix', text: 'No text in the app is now smaller than 11 pixels. Some was as small as 7, including clinician initials and money values, and the page scale drops on tablets which made it genuinely 7 pixels on the devices people actually carry' },
      { type: 'feature', text: 'The buddy board has a key. Red meant absent and amber meant a day off, but nothing on screen said so, which made a normal week look like an emergency next to a panel reporting no problems. Days off are now outlined rather than filled, so the two states differ by shape as well as colour and remain readable for anyone with colour blindness' },
      { type: 'tweak', text: 'Capacity tiles recoloured so the numbers on them are legible. The bright fills put white text as low as 2.15 to 1 where 4.5 is required. Sessions that are short of cover also carry a diagonal stripe now, so the state that needs action is not signalled by colour alone' },
      { type: 'tweak', text: 'The changelog inside the app is paginated, the same fix the public page received last release. It had been drawing every release at once, over 54000 pixels tall' },
    ]
  },
  {
    version: '4.120.0',
    date: '2026-08-30',
    title: 'Cross practice data leak closed, crash reporting, first unit tests',
    changes: [
      { type: 'fix', text: 'A signed in user could read any other practice staff list. The list_practice_members function runs with elevated rights and had lost its caller check during a rebuild in May, so passing any practice id returned that practice full member list including names, email addresses and last sign in times. The check is restored and the function carries a comment warning that any future edit must keep it. Every other elevated function that takes a practice id was audited and all of them were already correctly guarded' },
      { type: 'feature', text: 'Crashes now report themselves. A new errors screen in the admin area lists what actually broke in a browser, with the message, the page it happened on, the app version and the stack. Previously a crash showed a box asking someone to screenshot it and pass it on, and anything that failed quietly was never seen at all. Reports are stored in our own database, nothing is sent to an outside service, and no names or emails are recorded' },
      { type: 'feature', text: 'First unit tests, 41 of them, covering the logic that decides staffing and demand. These check the session bucket rules that EMIS exports use, the red amber green staffing thresholds, date parsing, slot ordering and demand scaling by list size. Run with npm run test:unit in under a second' },
      { type: 'fix', text: 'The impersonation banner was reading cookies the old way, which logged an error on every single page render and will stop working entirely on the next major framework upgrade. Since that red banner is the only thing telling an admin they are acting as someone else, it is not a component that can be allowed to fail quietly' },
      { type: 'fix', text: 'Four high severity dependency vulnerabilities patched, including a denial of service and a request forgery issue in the web framework. One remains that can only be cleared by a major framework upgrade, and it affects a build tool that never sees untrusted input' },
      { type: 'note', text: 'Removed a leftover diagnostic page that was publicly reachable and printed raw database error codes to anyone who visited it. Its own comment said it should have been deleted once the real pages existed' },
    ]
  },
  {
    version: '4.119.0',
    date: '2026-08-30',
    title: 'Design pass on every page a visitor sees before signing in',
    changes: [
      { type: 'fix', text: 'The sign in and sign up pages no longer describe GPDash as a preview for testing only. That line sat directly under the sign in heading on the live site, which is not what a practice should read while being asked for credentials' },
      { type: 'tweak', text: 'One accent colour now runs across the whole signed out journey. The legal, privacy, changelog and goodbye pages used cyan links while the sign in pages used green, so the colour language changed halfway through. The cyan used inside the app keeps its separate meaning and is untouched' },
      { type: 'fix', text: 'Small print now meets the accessibility contrast standard. There were eight failures, every one the same grey at 11 to 12 pixels sitting at 3.5 to 1 where 4.5 is required. All eight came from a single colour and all eight are now clear' },
      { type: 'feature', text: 'The changelog page is paginated. It had been rendering every release at once, 289000 pixels tall on a phone across 7585 elements, enough to crash a browser mid render. It now shows the twelve most recent releases with a link to the full history, 51 times shorter' },
      { type: 'tweak', text: 'Body text on the legal and privacy pages enlarged and the column narrowed, so the longest lines come back to a comfortable reading width. These are the pages an information governance officer reads most closely and they were the hardest to read' },
      { type: 'tweak', text: 'Form inputs raised to 16 pixels, which stops mobile Safari zooming the whole page the moment a field is focused. Standalone links in the sign in footer given a full size touch target, and the three competing links there now have a clear order' },
      { type: 'note', text: 'Scoped to the signed out pages only. Nothing behind the login was reviewed or changed' },
    ]
  },
  {
    version: '4.118.0',
    ...
    changes: [
      { type: 'feature', text: 'Playwright end to end tests added, the first automated tests in the project. They cover the security headers on every response, the routing chain an anonymous visitor walks from the front door to the login page, and the shape of the one endpoint served without authentication. Run them with npm run test:e2e against a local dev server, or point them at a deployed site with an environment variable' },
      { type: 'feature', text: 'The header test was checked by deliberately deleting a policy directive and confirming the test failed, then restoring it. A test that cannot fail is not worth having, so the guard is known to bite rather than merely known to pass' },
      { type: 'note', text: 'One test is skipped by default because it needs a practice that has opted into the public buddy board. Supply that slug in an environment variable and it asserts that no internal clinician field appears in the live public payload' },
    ]
  },
  {
    version: '4.117.2',
    date: '2026-08-30',
    title: 'Five security and data handling fixes found in a code review',
    changes: [
      { type: 'fix', text: 'The public buddy board was returning internal clinician fields to anyone holding the link. Notes written in the admin panel, long term absence markers and internal user ids all sat in the payload even though the public board never shows them. The response now carries only the seven fields the board actually reads, built as an allow list so a field added later cannot leak by default' },
      { type: 'fix', text: 'Admin sign in links were assembled from browser supplied headers rather than the configured site address, so a link could be bound to a temporary deployment host that later stops resolving. Both admin link routes now use the shared site URL helper that was written for exactly this problem' },
      { type: 'fix', text: 'Retention cleanup could fail on every table and still report success with zero rows deleted, which reads identically to a healthy run with nothing to delete. It now refuses to start without a service key, and a run where every table errored is reported as a failure rather than silently leaving personal data in place' },
      { type: 'fix', text: 'Account deletion reported complete erasure even when the email scrub failed. The database layer returns an error rather than raising one, so the failure was invisible to the check that was there. Failures are now detected and the response names any column that may still hold an identifier' },
      { type: 'fix', text: 'A rate limit response passed a third argument to a helper that accepts two, so its headers were silently discarded. Folded into the real headers, and the cache setting corrected so a rate limit rejection is never stored by an edge cache' },
    ]
  },
  {
    version: '4.117.1',
    date: '2026-08-30',
    title: 'Local development no longer blocked by the script security policy',
    changes: [
      { type: 'fix', text: 'The browser security policy that limits which scripts may run also blocked the local development server. Fast refresh has to evaluate code at runtime and the policy forbade it, so a developer machine would load the dashboard once and then break on the first edit. Development now permits it. The live site policy is unchanged and still forbids runtime evaluation, confirmed by generating the header in both modes and comparing' },
    ]
  },
  {
    version: '4.117.0',
    date: '2026-08-23',
    title: 'Capacity page: session-level staffing and a week detail view',
    changes: [
      { type: 'feature', text: 'Site staffing boxes redesigned. The All total sits on its own row above the sites, site identity is now an edge stripe so the fill colour means only one thing, capacity. Green is headroom, amber is exactly on minimum, red is short and shows the shortfall' },
      { type: 'feature', text: 'Staffing is now judged per session, not per day. Each site box shows its morning and afternoon split and its colour follows the worst session with activity there, so a strong morning no longer hides a one person afternoon. The hover explains which session set the colour' },
      { type: 'feature', text: 'New week detail view on the capacity page, toggle between the 6 week calendar and a full week breakdown. Each day shows every site, every session, who is offering appointments with urgent and routine slot counts, duty doctor starred, and clinicians with no bookable slots dimmed. Sessions below minimum are named in a strip at the top' },
      { type: 'feature', text: 'Days beyond the EMIS export fall back to a rota projection card listing scheduled GPs per session, clearly marked as rota derived rather than pretending to know slot numbers' },
    ]
  },
  {
    version: '4.116.2',
    date: '2026-08-10',
    title: 'Crashes now show their cause instead of a blank page',
    changes: [
      { type: 'fix', text: 'A crash reported while scrolling buddy cover showed only the generic blank application error page. Sections are now wrapped in an error boundary, if anything crashes, the screen shows the actual error message with a note to screenshot it and a try again button, instead of losing the whole app. The appointment hover popup is also hardened so it can never take the page down' },
    ]
  },
  {
    version: '4.116.1',
    date: '2026-08-10',
    title: 'Bank holiday detection done properly',
    changes: [
      { type: 'fix', text: 'The bank holiday guard was counting EMIS rows, and a holiday skeleton carries synthetic entries like Unknown and TRIAGE TELEPHONE that beat the count. The test is now whether anyone on the actual staff register has sessions that day, verified against the real 31 August data, nobody does, so the day is treated as closed and no false inconsistencies are raised' },
    ]
  },
  {
    version: '4.116.0',
    date: '2026-08-10',
    title: 'Long term absence truly persists, bank holidays stop crying wolf',
    changes: [
      { type: 'fix', text: 'Long term absence and leaving markers are now persisted by the server as part of every save, not by a separate browser write that could fail silently. The database showed zero stored markers despite the earlier fix, the API layer had not picked up the new column, its schema cache has been reloaded and the server now writes the marker on any change, so it survives however the change is made, including the automatic sweep' },
      { type: 'fix', text: 'On a bank holiday or any closed day, when EMIS shows essentially nobody with sessions, the inconsistency panel no longer flags everyone as marked present with no sessions. A closed practice is not an inconsistency' },
    ]
  },
  {
    version: '4.115.1',
    date: '2026-08-10',
    title: 'Appointment popup follows the mouse',
    changes: [
      { type: 'fix', text: 'The EMIS appointments popup on inconsistency rows now appears right at the cursor rather than centred on the row, which on wide rows put it a long way from the mouse' },
    ]
  },
  {
    version: '4.115.0',
    date: '2026-08-10',
    title: 'Hover an inconsistency to see the actual EMIS appointments',
    changes: [
      { type: 'feature', text: 'On buddy cover inconsistencies, hovering a marked absent but EMIS has sessions row now shows exactly what EMIS holds for that clinician that day, slot types with counts and AM or PM, so a day of 3x CPD is instantly distinguishable from a real clinical session before you decide' },
    ]
  },
  {
    version: '4.114.1',
    date: '2026-08-10',
    title: 'Public board shows real allocations, cards truly tidy',
    changes: [
      { type: 'fix', text: 'The public buddy board was reading allocations from a table that does not exist, so it always said no allocations yet. It now reads the real store and shows the actual cover' },
      { type: 'fix', text: 'Attendance cards now share a uniform height, and the status badge line, leaving countdowns, holidays, half days, spans the full card width below the name row, so nothing can collide with the Present and Absent control and badges are no longer cut short' },
    ]
  },
  {
    version: '4.114.0',
    date: '2026-08-10',
    title: 'Half sessions and a staff filter on the working days grid',
    changes: [
      { type: 'feature', text: 'Sessions on the working days grid now cycle off, full, half, shown as a half filled square. A half session still counts as working everywhere, presence, buddy cover, the boards, but session counts now step in halves so a clinicians total can match their contract, for example 5.5 sessions. Halves survive edits made in other rota editors' },
      { type: 'feature', text: 'The working days grid gains a staff filter, defaulting to buddy cover clinicians, with All staff, GPs, Nursing, HCAs and Other options' },
    ]
  },
  {
    version: '4.113.1',
    date: '2026-08-10',
    title: 'Calendar subscribe button fixed',
    changes: [
      { type: 'fix', text: 'Fetching a calendar feed link always failed with a misleading permissions message, even for the linked user. A parameter check helper was used wrongly so every valid request looked parameterless, the endpoint had never succeeded for anyone. Fixed, and the button now distinguishes a real permission refusal from other errors, which show their cause instead of blaming permissions' },
    ]
  },
  {
    version: '4.113.0',
    date: '2026-08-10',
    title: 'Tidier buddy cover cards',
    changes: [
      { type: 'improvement', text: 'Clinician cards on buddy cover are restructured so nothing overlaps the Present and Absent control. Name and flags on the first line, role and usual sessions on the second, and a new third line carries the detail badges, the Leaving countdown with its adjust and undo menu, half day chips, holiday or other leave with its end date, and long term absence' },
    ]
  },
  {
    version: '4.112.2',
    date: '2026-08-10',
    title: 'Working days grid labels and a link from buddy cover',
    changes: [
      { type: 'fix', text: 'The working days grid header now labels all three columns, AM, PM and Eve, and its description reflects the three session model' },
      { type: 'improvement', text: 'The buddy cover page now links straight to the working days grid, Edit working days, next to the weekly inconsistencies panel' },
    ]
  },
  {
    version: '4.112.1',
    date: '2026-08-10',
    title: 'Evening session everywhere it can be edited',
    changes: [
      { type: 'improvement', text: 'The working days grid on the practice page and in the setup wizard now shows all three session toggles, morning, afternoon and evening, matching the unified rota. Session counts include evenings. Filling a whole row keeps any existing evening setting rather than switching evenings on for everyone' },
    ]
  },
  {
    version: '4.112.0',
    date: '2026-08-10',
    title: 'The three session rota is now the single source of truth',
    changes: [
      { type: 'feature', text: 'The working rota is now defined in Morning, Afternoon and Evening sessions per weekday, one authoritative model that everything reads. The old day level rota still exists but only as a derived view for display. Buddy cover status, the boards, Whos In, the public page, inconsistency detection and Locum spend all now answer the same question from the same place' },
      { type: 'improvement', text: 'TeamNet leave sits on top of the session rota properly: someone is absent for the day only when leave covers every session they ordinarily work. A morning only clinician with a morning TeamNet entry is absent, an all day clinician with the same entry is in for the afternoon' },
      { type: 'improvement', text: 'The grid in Practice, Clinicians now edits the rota directly, click any M A E segment to toggle it, click the day dot to switch a whole day. Amber now means the EMIS history disagrees with the rota or is uncertain, so review those first. A Fill from EMIS history button sets every clinicians sessions from what actually happens, and everything is audited' },
    ]
  },
  {
    version: '4.111.0',
    date: '2026-08-10',
    title: 'Has left finally persists, and any session present counts as in',
    changes: [
      { type: 'fix', text: 'Has left and Long term absence markers were never reaching the database. The bulk save deliberately refuses clinician field updates, a protection added after an old data loss incident, so the marker lived only in the browser session and the prompts returned after every reload. Status changes now also write directly to the clinician record, the automatic sweep does the same, and undo and adjust work even after a reload' },
      { type: 'improvement', text: 'One source rule applied across the board: someone present for any session of the day counts as in. A TeamNet half day entry no longer marks the whole day absent, buddy cover is not allocated for them, the public board and Whos In show them as in, and the half day chip still shows the detail' },
      { type: 'fix', text: 'Absence origin and half day markers now survive a reload, previously the save path silently discarded them, which could also make synced TeamNet leave indistinguishable from manual entries' },
    ]
  },
  {
    version: '4.110.0',
    date: '2026-08-10',
    title: 'Buddy cover regenerates itself',
    changes: [
      { type: 'feature', text: 'The generate next 4 weeks button is now largely retired from daily use. Whenever anything that affects cover changes, marking someone as left or long term absent, changing a days presence, adding or removing leave, editing working days, closing a day, or the morning TeamNet sync, the next four weeks of buddy cover regenerate automatically a moment later, with a quiet confirmation toast. The button remains as a manual backstop' },
      { type: 'improvement', text: 'Regeneration is now stable and respectful: a day is only recalculated when that days own situation changed, so unrelated edits never reshuffle cover people have already seen, and manual reassignments are re applied automatically where the chosen coverer is still in that day, dropped only when the new situation makes them impossible' },
    ]
  },
  {
    version: '4.109.1',
    date: '2026-08-10',
    title: 'Buddy panel crash fixed',
    changes: [
      { type: 'fix', text: 'The buddy page could crash with a reference error introduced with the half day leave awareness, the covering absence helper called an array function that does not exist in that file. It compiled and passed checks but failed the moment it ran. Fixed with a self contained guard and, per the standing lesson, proven by executing it against real shaped data before shipping' },
      { type: 'fix', text: 'The appointment data endpoint now verifies membership and retries with elevated access when a permission scoped read transiently returns nothing, so a session hiccup can no longer blank the board, previously it survived only via the slower fallback path' },
    ]
  },
  {
    version: '4.109.0',
    date: '2026-08-09',
    title: 'One shared session pattern across the grid, buddy cover and spend',
    changes: [
      { type: 'feature', text: 'The working days grid in Practice, Clinicians now shows Morning, Afternoon and Evening segments under each working day, generated automatically from EMIS history. An amber outline marks where the history is uncertain, for example a fifty fifty Tuesday, so the eye goes straight to days that may need a correction. Click a segment to pin it on or off, pins show in indigo, every change is audited, and hovering shows the evidence, seen on 75 percent of the last 8 Mondays' },
      { type: 'improvement', text: 'Locum spend now judges extra sessions against exactly this shared pattern, including your pins, so a review card and the grid can never disagree. The buddy board also shows each clinicians usual sessions for the day, such as AM only, next to their name' },
    ]
  },
  {
    version: '4.108.0',
    date: '2026-08-09',
    title: 'Personal calendar feeds, smarter inconsistencies, half day leave',
    changes: [
      { type: 'feature', text: 'Every clinician now has a personal calendar subscription. On My Rota, Subscribe to this calendar gives a link for Google, Apple or Outlook, each working session appears with the site in the title, the location field set, and session times so the duration shows. Available to the linked user or an admin, secured by an unguessable per clinician link, and it updates as EMIS data is uploaded' },
      { type: 'feature', text: 'The inconsistency panel now works in both directions. When EMIS regularly shows booked sessions on a day that is not one of a clinicians working days, a suggestion works out which day it is and offers to add it to their working days with one click, audited, with an ignore option' },
      { type: 'improvement', text: 'TeamNet half day leave is now understood. An entry like Fri 28 Aug Afternoon marks only the afternoon, so a morning EMIS session no longer raises a false inconsistency, and the buddy board shows a PM off in AM style chip for the day' },
      { type: 'improvement', text: 'Long term sick is renamed Long term absence throughout' },
    ]
  },
  {
    version: '4.107.0',
    date: '2026-08-09',
    title: 'Site staffing rebuilt properly',
    changes: [
      { type: 'fix', text: 'Staffing counts showed zero for every site because the routine slot category was empty for this practice, the routine definition lives in the saved routine filter instead. The count now reads both, so it works wherever a practice keeps its routine config' },
      { type: 'fix', text: 'Minimums were not saving between sessions because they were stored in a field the save route silently dropped. They now live inside the huddle settings, which persist properly. Any minimums entered before this fix were lost and need entering once more' },
      { type: 'improvement', text: 'The prompt boxes are gone. The Minimums button now opens a proper settings panel with a dropdown for the whole practice and each site, plus checkboxes for who counts toward staffing, GPs by default, judged from roles on the staff register' },
      { type: 'improvement', text: 'The hover over each site box is now a styled popup matching the rest of the app, showing the count against the minimum, who is counted, and who is based there with no routine slots' },
    ]
  },
  {
    version: '4.106.3',
    date: '2026-08-09',
    title: 'September fix now actually applied to the 30 day chart',
    changes: [
      { type: 'fix', text: 'The September date fix announced in 4.106.1 never actually reached the Next 30 days routine chart. The build script that applied it silently skipped the step, a shell quirk where a passing check reports failure, so the chart kept building lookup keys with the British Sept abbreviation and showed every September day as empty. The fix is now genuinely in the code, verified by running the real appointment file through the real chart calculation, September now shows its true numbers' },
      { type: 'fix', text: 'The 92 day forward cap on CSV merges from 4.106.1 was skipped by the same fault and is now genuinely applied as well, complementing the server side cap from 4.106.2' },
    ]
  },
  {
    version: '4.106.2',
    date: '2026-08-09',
    title: 'Oversized appointment data can no longer be saved',
    changes: [
      { type: 'fix', text: 'The appointment data retention window, four months back and 92 days forward, is now enforced on the server at save time, not just in the app at upload time. Previously a still open session holding an oversized dataset would silently re-upload the whole thing on its next save, undoing any cleanup. This happened live: an eight year export re-appeared minutes after being trimmed, pushing the data past the hosting response size limit and leaving dashboards stuck on stale appointments. A stale re-send now trims to identical content and becomes a harmless no op' },
    ]
  },
  {
    version: '4.106.1',
    date: '2026-08-09',
    title: 'September was invisible',
    changes: [
      { type: 'fix', text: 'Every view that looks up appointment data by date was blind to September. British locale formatting abbreviates September as Sept, four letters unlike every other month, so the app built lookup keys like 01-Sept-2026 that could never match the EMIS format 01-Sep-2026. Date keys are now built from a fixed month list. This bug could not show itself until August, when the 30 day routine window first reached into September' },
      { type: 'fix', text: 'CSV uploads from an open ended EMIS search no longer balloon the stored data. One upload arrived spanning eight years of rota templates. Merges now keep 92 days forward, so the next normal upload automatically shrinks the store back down' },
    ]
  },
  {
    version: '4.106.0',
    date: '2026-08-09',
    title: 'Spring clean - the last of v3 removed',
    changes: [
      { type: 'tech', text: 'The legacy v3 single password front page and its two supporting endpoints, the old shared data store route and the old TeamNet sync route, have been deleted. The site root now always goes to the v4 sign in' },
      { type: 'tech', text: 'The one time v3 to v4 import page and endpoint are gone, the migration is long complete. The disabled practice reset branch in Buddy settings is also removed' },
      { type: 'tech', text: 'Middleware simplified, the emergency switch back to v3 no longer exists because there is no v3 to switch back to. The APP_PASSWORD and NEXT_PUBLIC_DEFAULT_TO_V4 environment variables are no longer read anywhere and can be removed' },
    ]
  },
  {
    version: '4.105.0',
    date: '2026-08-09',
    title: 'Spend review in one click, exact rate lines, visible rate history',
    changes: [
      { type: 'feature', text: 'The review queue gains a one click bulk action to mark every likely swap as not an extra. Each decision is still stored individually so any one can be undone from Recent decisions, and the audit log records a single summary entry rather than drowning in duplicates' },
      { type: 'improvement', text: 'When a locum rate changes mid month their monthly breakdown now shows one exact line per rate, for example 6 sessions at the old rate and 4 at the new, instead of a blended average' },
      { type: 'improvement', text: 'Rate history is now visible. Hover a name in the rates editor to see every dated rate step, and a small note flags anyone with more than one rate change' },
    ]
  },
  {
    version: '4.104.0',
    date: '2026-08-09',
    title: 'Wind-down countdowns, whole-practice staffing, spend accuracy',
    changes: [
      { type: 'feature', text: 'Wind-down labels now count down, for example Leaving 3 wks left, and a Wind-downs in progress note in the buddy panel lists everyone mid wind-down with their end date. Clicking a label opens a small menu to adjust the end date or undo the status, both audited, so extending a notice period no longer needs an undo and re-set' },
      { type: 'feature', text: 'Capacity staffing gains a Whole practice box shown first in each day, clearly differentiated in indigo, counting each clinician once across all sites with its own minimum. The Site staffing button also shows an amber badge with the number of days in the next six weeks below any minimum, so warnings are visible without opening the layer' },
      { type: 'feature', text: 'Locum spend now catches people who worked sessions but cannot be classified, either not on the staff register or with no role set, with a button straight to the staff register to fix them. These are very often ad hoc locums and were the biggest hole in the spend figure' },
      { type: 'improvement', text: 'Rates are now effective dated. Changing a rate applies from today and earlier months keep the rate that was in force at the time, so the finance figures never silently rewrite history. A month on month trend chart and a CSV export for your accountant complete the page' },
    ]
  },
  {
    version: '4.103.1',
    date: '2026-08-09',
    title: 'Wind-down error alerts, undo everywhere, full audit trail',
    changes: [
      { type: 'fix', text: 'Corrected logic: someone who has left should not be in EMIS at all. If a leaver in wind-down still shows booked EMIS sessions, the buddy panel now raises a red warning card saying something has gone wrong, with a one-click undo, instead of silently suppressing it' },
      { type: 'feature', text: 'Undo for status transitions. Click the greyed Leaving wind-down or Long-term sick label on a clinician, or the button on a warning card, to cancel the status - the cover absence is removed and they return to normal' },
      { type: 'feature', text: 'Undo for locum spend decisions. A Recent decisions list shows who decided what and when, and undo returns the session to the review queue' },
      { type: 'improvement', text: 'Everything is now audited: setting or undoing a status, automatic sweep changes, every spend review decision and undo, and every rate change all write to the audit log with who and when' },
    ]
  },
  {
    version: '4.103.0',
    date: '2026-08-09',
    title: 'Wind-down fixes, site staffing, locum spend tracker',
    changes: [
      { type: 'fix', text: 'Setting Has left or Long-term sick no longer re-asks every week. While a wind-down is active the clinician is suppressed from the inconsistency and suggestion panels, shown greyed on the buddy board with a Leaving wind-down or Long-term sick label, and then automatically marked as left and removed when the period ends' },
      { type: 'feature', text: 'Capacity planning gains a Site staffing toggle. Each day can show one box per site with the site letter, site colour and a count of clinicians who have routine slots at that site that day. People based at a site with no routine slots do not inflate the count and appear in the hover instead. A per site minimum can be set via the Minimums button and a box below its minimum turns amber with the reason explained on hover. Sites come from the database so this works for any practice' },
      { type: 'feature', text: 'New Locum spend page. Month on month spend from locum sessions at per locum rates plus regular GPs doing confirmed extra sessions. Sessions are detected from EMIS appointment times using a morning afternoon evening model with deliberate buffer zones so duty overlaps never misclassify. A review queue flags sessions outside a GPs usual pattern with swap detection when the weekly total matches their normal, and every decision is stored so nothing is asked twice' },
      { type: 'fix', text: 'The bare buddy web address used by historic EMIS links now redirects to the public buddy board when exactly one practice has the board enabled, instead of showing a not found page' },
    ]
  },
  {
    version: '4.102.2',
    date: '2026-06-19',
    title: 'Public buddy board fixed',
    changes: [
      { type: 'fix', text: 'The public buddy board was refusing every visit with a too many requests message because of a one word mistake in its traffic check, it had never actually worked. Fixed, and the board is now also served from a cache so a whole practice sharing one internet connection, including waiting room screens, never trips the limit. The same mistake also silently blocked the account data export, which now works too' },
    ]
  },
  {
    version: '4.102.1',
    date: '2026-06-19',
    title: 'Clearer public board errors',
    changes: [
      { type: 'tweak', text: 'When the public buddy board cannot load it now says plainly what is wrong and what to do, for example incomplete site setup versus no internet connection, instead of a generic unable to load message with the real cause in tiny print' },
    ]
  },
  {
    version: '4.102.0',
    date: '2026-06-18',
    title: 'Smarter inconsistency suggestions',
    changes: [
      { type: 'tweak', text: 'When a clinician has no EMIS sessions on any of their days this week, the panel now shows one clear suggestion card for them instead of a row per day, and proposes setting a status. A single Set status menu offers Has left and Long term sick with a plain explanation of each, replacing the two separate buttons. One off single day mismatches keep the simple Present and Absent choices' },
    ]
  },
  {
    version: '4.101.0',
    date: '2026-06-18',
    title: 'Wind down leavers and long term sickness',
    changes: [
      { type: 'feature', text: 'When the inconsistency panel shows a clinician with no EMIS sessions, you can now mark them as Has left, which keeps them covered by the buddy system for a wind down period, normally about two months while results and letters continue to arrive, and then automatically marks them as left' },
      { type: 'feature', text: 'You can also mark them as Long term sick for an estimated period. If EMIS starts showing booked sessions for them before the period ends, they are automatically marked as back and their absence is closed' },
      { type: 'fix', text: 'The TeamNet sync previously replaced the whole absence list each morning, which would have wiped manually recorded absences. It now only refreshes its own entries, manual records and wind downs are preserved' },
    ]
  },
  {
    version: '4.100.0',
    date: '2026-06-18',
    title: 'Two factor aware password changes and friendlier errors',
    changes: [
      { type: 'feature', text: 'Changing your password when your account has two factor authentication now simply asks for your 6 digit authenticator code on the same screen, instead of failing with a technical error' },
      { type: 'tweak', text: 'Sign in and security errors across the site now explain what happened in plain language and tell you what to do next. The old technical messages are translated automatically' },
      { type: 'fix', text: 'Revoking an invite now removes it from the pending invites list immediately. Previously it stayed listed even though it had been revoked' },
    ]
  },
  {
    version: '4.99.0',
    date: '2026-06-18',
    title: 'Invite emails and clearer roles',
    changes: [
      { type: 'feature', text: 'Inviting a colleague now sends them a proper GPDash invitation email automatically, with an accept button and the link as fallback. The copy link button remains if you also want to send it yourself' },
      { type: 'tweak', text: 'Choosing a role when inviting now shows every role with a one line explanation of what it gives access to, instead of hiding the detail behind a dropdown' },
    ]
  },
  {
    version: '4.98.1',
    date: '2026-06-18',
    title: 'New login is now the front door',
    changes: [
      { type: 'tweak', text: 'Visiting gpdash.net now goes straight to the new sign in screen. The old single password page no longer greets anyone' },
    ]
  },
  {
    version: '4.98.0',
    date: '2026-06-18',
    title: 'Password reset by 6 digit code',
    changes: [
      { type: 'feature', text: 'Password reset now works by entering the 6 digit code from the email as well as by link. NHS mailboxes scan emailed links for viruses and the scan uses up the one time link before you can click it, but a code cannot be used up by a scanner, so it always works' },
    ]
  },
  {
    version: '4.97.1',
    date: '2026-06-18',
    title: 'Fix clinicians wrongly marked away after leave',
    changes: [
      { type: 'fix', text: 'The buddy system was marking clinicians as away on their first working day back after a block of leave, blaming a multi day leave block even though their leave had ended. The rule is now exactly as it should be, on a working day a clinician is present unless TeamNet leave covers that specific date or they are flagged long term absent. Day off versus absent behaviour, including flagging a day off for cover when it sits next to recorded leave, is unchanged' },
    ]
  },
  {
    version: '4.97.0',
    date: '2026-06-18',
    title: 'Rooms hidden for now',
    changes: [
      { type: 'tweak', text: 'The Rooms page is hidden from the sidebar until the module is further developed. Nothing is deleted, it will return when ready' },
    ]
  },
  {
    version: '4.96.2',
    date: '2026-06-18',
    title: 'Today page data now survives refresh',
    changes: [
      { type: 'fix', text: 'Found and fixed the actual cause of appointment data disappearing on refresh. An internal marker telling the app to fetch your data was being silently stripped by a data translation layer, so the fetch never ran at all. The marker now survives translation and, belt and braces, the app fetches your data whenever the page arrives without it, marker or not. Your previously uploaded data should reappear by itself, no re-upload needed' },
    ]
  },
  {
    version: '4.96.1',
    date: '2026-06-18',
    title: 'Fix Today page data disappearing on refresh',
    changes: [
      { type: 'fix', text: 'Uploaded appointment data could vanish from the Today page after a refresh. The loader that fetches your appointment data now retries on failure and falls back to the original proven data source if the fast path returns nothing, so the data always comes back. If the fast path misbehaves it also records exactly what happened so the underlying cause can be pinned down' },
    ]
  },
  {
    version: '4.96.0',
    date: '2026-06-18',
    title: 'Buddy Cover refinements',
    changes: [
      { type: 'fix', text: 'The hover explanation box on clinician pills no longer drifts off the edge of the screen, it now measures where it actually landed and pulls itself back into view on every browser' },
      { type: 'tweak', text: 'The inconsistency panel now lives in a proper column on the right on larger screens. It always shows its status, including a green all clear when the board matches EMIS, and decisions already made stay listed greyed out with who decided, what they chose and when' },
      { type: 'feature', text: 'TeamNet now syncs automatically once a day when the dashboard is opened, so planned absences stay current without pressing the Sync button. The manual button still works as before' },
    ]
  },
  {
    version: '4.95.0',
    date: '2026-06-18',
    title: 'Buddy Cover: audit trail and inconsistency review',
    changes: [
      { type: 'feature', text: 'Every manual presence change on the Buddy Cover page is now stamped with who made it and when, shown at the bottom of the hover explanation on each clinician' },
      { type: 'feature', text: 'A new Review this weeks inconsistencies panel sits under the header whenever the board and the EMIS appointment book disagree, for example someone marked present with no booked sessions. Each row offers a Present or Absent decision, the decision is recorded with your name and the time, and the row clears once decided' },
      { type: 'tweak', text: 'The Buddy Cover page now renders about ten percent smaller on desktop screens so more of the week fits comfortably' },
    ]
  },
  {
    version: '4.94.0',
    date: '2026-06-18',
    title: 'Instant splash on app launch',
    changes: [
      { type: 'tweak', text: 'The home screen app now starts on a dedicated launch screen that your phone can keep in its local cache, so the GPDash splash appears the moment you tap the icon, before any network activity, and hands straight over to your dashboard. To get this you need to remove the GPDash icon from your home screen once and add it again, because phones remember the start address from when the icon was first added' },
    ]
  },
  {
    version: '4.93.0',
    date: '2026-06-18',
    title: 'Much faster app launch and a proper loading screen',
    changes: [
      { type: 'tweak', text: 'The dashboard no longer downloads your entire appointment dataset before it can appear. That data was the bulk of the long black screen when launching on a phone, and it now loads in the background after the app is already on screen, with the Today page filling in as soon as it arrives, usually within a second or two' },
      { type: 'tweak', text: 'Launching the app now shows a proper GPDash loading screen with the logo and a spinner instead of a black screen, and the fast launch path now works reliably in the home screen app as well as the browser' },
    ]
  },
  {
    version: '4.92.0',
    date: '2026-06-18',
    title: 'Design system unification complete',
    changes: [
      { type: 'tech', text: 'Over seven hundred styling declarations across the whole site were migrated onto the unified design system, every one verified to render identically. The remaining inline styling is genuinely dynamic content, which is the documented exception, and the login pages are formally documented as the intentionally dark auth theme. One regression the migration itself introduced, dropped line heights on around fifty text blocks, was caught by the verification checks and fixed before release' },
    ]
  },
  {
    version: '4.91.0',
    ...
    changes: [
      { type: 'tech', text: 'The three styling approaches that had grown up across the site now share a single source of truth. Every design token, colours, surfaces, text scale, borders, is wired into the styling framework so both worlds draw the same values, the target way of writing styles is documented in the stylesheet, and the first module has been migrated as the living template. Remaining modules will follow one at a time with checks on preview between each' },
    ]
  },
  {
    version: '4.90.0',
    ...
    changes: [
      { type: 'tweak', text: 'A sweep of typography and styling across the whole site. Dozens of slightly off font sizes were aligned to a consistent type scale, corner rounding now follows the shared design tokens everywhere it had drifted, and the automated import panel was rebuilt on the shared button and spacing system so it matches the rest of the site' },
      { type: 'tech', text: 'The design scale for type, spacing, corner radii, surfaces and buttons is now documented at the heart of the stylesheet so all future work follows one standard' },
    ]
  },
  {
    version: '4.89.0',
    ...
    changes: [
      { type: 'fix', text: 'A full automated and manual review of the codebase. Fixed two silent style bugs where duplicate settings overwrote each other in the clinician quick setup table and the workforce planner defaults. The review also verified the areas most likely to hide problems, including data saving calls, date handling and the new launch fast path, and found them clean' },
      { type: 'tech', text: 'Added a linting setup to the project so the same automated bug checks can be run on every future change' },
    ]
  },
  {
    version: '4.88.0',
    date: '2026-06-17',
    title: 'Much faster app launch',
    changes: [
      { type: 'tweak', text: 'Launching GPDash from the home screen used to bounce through three separate server steps before your dashboard could even begin loading, each one checking your login again. The app now remembers your practice and goes straight to your dashboard in a single step, cutting several seconds from a cold launch on mobile' },
    ]
  },
  {
    version: '4.87.0',
    date: '2026-06-17',
    title: 'Instant first screen when launching the app',
    changes: [
      { type: 'tweak', text: 'Launching GPDash, especially from the home screen on a phone, used to show a blank screen until the server had finished loading all your data. The app now paints a placeholder screen immediately and fills in the dashboard as soon as the data arrives, so it is visible from the first moment instead of appearing frozen' },
    ]
  },
  {
    version: '4.86.1',
    date: '2026-06-17',
    title: 'Quicker first interactions',
    changes: [
      { type: 'tweak', text: 'The connection to the database service is now warmed up while the page loads, so the first action that talks to it, such as opening Meetings or the action register, no longer pays a connection delay. Most noticeable on mobile' },
    ]
  },
  {
    version: '4.86.0',
    ...
    changes: [
      { type: 'tweak', text: 'Fonts are now bundled with the app instead of loading from an external service. That removes the last third party connection on the entire site, which saved a noticeable delay on slower mobile connections, and font files are now cached permanently by the browser' },
      { type: 'tweak', text: 'The Today page code now starts downloading the moment the dashboard loads rather than waiting its turn, removing a network round trip from the daily route. Most noticeable on phones' },
    ]
  },
  {
    version: '4.85.0',
    date: '2026-06-17',
    title: 'Site speed and visual consistency',
    changes: [
      { type: 'tweak', text: 'A performance pass across the whole site. The home page now loads about 75 percent less code up front, with each section loading on demand, and fonts no longer block the first paint of any page. Pages should feel noticeably snappier, especially on mobile connections' },
      { type: 'fix', text: 'Fixed a styling gap where the Meetings cards and the automated import panel referenced colour tokens that were never defined, leaving those cards without their proper background fill. They now match the rest of the site in both dark and light themes, and the Meetings heading uses the correct heading font' },
    ]
  },
  {
    version: '4.84.1',
    date: '2026-06-16',
    title: 'Fix Today page stat tiles on iPad',
    changes: [
      { type: 'fix', text: 'On iPad the headline stat tiles on the Today page were squashed into narrow strips, wrapping every word and clipping the big numbers. The Today panels now use the full width and stack neatly until the screen is genuinely wide enough for the four column desktop layout, keeping the numbers readable' },
    ]
  },
  {
    version: '4.84.0',
    date: '2026-06-16',
    title: 'Better layout on iPad and tablets',
    changes: [
      { type: 'tweak', text: 'The whole site now scales more sensibly on tablets. The 125 percent zoom that suits a desktop monitor was crammed on iPad, so it now eases off on smaller screens. The Today page also reflows to two columns on iPad portrait instead of squeezing the full four-column desktop layout' },
    ]
  },
  {
    version: '4.83.0',
    date: '2026-06-16',
    title: 'Meetings on iPad and touch',
    changes: [
      { type: 'tweak', text: 'Made the Meetings module easier to use on iPad and touch screens. Action tick boxes, status toggles, assignee and date controls are now larger and easier to tap, the tabs scroll sideways instead of cramping, and the action rows have more breathing room' },
    ]
  },
  {
    version: '4.82.1',
    date: '2026-06-15',
    title: 'Fix practice page query and migration pipeline',
    changes: [
      { type: 'fix', text: 'Fixed a database query on the practice page that referenced a column by the wrong name, which was causing errors when loading pending invites' },
      { type: 'tech', text: 'Pinned the Supabase CLI version in the migration pipeline so a broken upstream release can no longer cause migration runs to fail' },
    ]
  },
  {
    version: '4.82.0',
    date: '2026-06-14',
    title: 'Automated demand import',
    changes: [
      { type: 'feature', text: 'You can now set up automated import of your demand CSV. Create a secure token on the practice page, then have a tool like Power Automate post your daily export to GPDash automatically. Re-importing a date safely replaces that day, and every import is logged so you can see when it last ran and how many rows came in' },
      { type: 'tech', text: 'New authenticated ingest endpoint scoped per practice by a hashed token, reusing the existing demand parsers and the idempotent demand_history upsert. Tokens are leadership only and grant no access to confidential data' },
    ]
  },
  {
    version: '4.81.0',
    date: '2026-06-14',
    title: 'Delete meetings, items and actions',
    changes: [
      { type: 'feature', text: 'You can now delete a meeting, an agenda item, or an action. Each asks you to confirm first and explains what will be removed. Deleting a meeting also removes its agenda items, but any open actions it raised are kept in the action register so nothing important is lost' },
    ]
  },
  {
    version: '4.80.0',
    date: '2026-06-14',
    title: 'Clearer action completion',
    changes: [
      { type: 'fix', text: 'Reworked how actions are completed. There is now a clear tick box to mark an action done, a separate Mark in progress control, and completed actions stay visible greyed out and struck through rather than disappearing. Every completed action has an Undo so a mis-tap is easy to reverse. Completed actions sink to the bottom and clear out next time the list loads' },
    ]
  },
  {
    version: '4.79.0',
    date: '2026-06-14',
    title: 'Actions carry forward between meetings',
    changes: [
      { type: 'feature', text: 'Open actions now carry forward. When you open a meeting, any actions still open or in progress from previous meetings appear at the top under Open actions carried forward, so nothing gets lost between meetings. You can update or complete them right there, and for recurring meetings only that series actions carry across' },
    ]
  },
  {
    version: '4.78.0',
    ...
    changes: [
      { type: 'tweak', text: 'Added confirmation messages when you create a meeting, complete an action, file an imported meeting or generate scheduled dates, plus smoother loading placeholders while meetings and actions load' },
    ]
  },
  {
    version: '4.77.1',
