...
export function canEditRotaNote(data, clinicianId) {
  ...
  //
  // Both sides have to actually exist. A bare === was true for an unlinked
  // user asked about an undefined clinician, because undefined === undefined,
  // which opened the note for someone who is linked to nobody.
  const linked = data?._v4?.linkedClinicianId;
  return !!linked && !!clinicianId && linked === clinicianId;
}
