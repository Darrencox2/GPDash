...
// The four capacity states. These were hardcoded to the dark palette - pale
// foregrounds on 14-20% tints - with no light values and no theme branch, so
// on a light card they rendered at about 1.4:1 against the 7.5:1 they manage
// on the dark page. They now read CSS custom properties, which carry a value
// per theme; the dark values behind them are unchanged. Only consumer is
// HuddleForward (browser), so var() is safe here.
export const STATE_COLOURS = {
  short: { bg: 'var(--st-short-bg)', bd: 'var(--st-short-bd)', fg: 'var(--st-short-fg)' },
  tight: { bg: 'var(--st-tight-bg)', bd: 'var(--st-tight-bd)', fg: 'var(--st-tight-fg)' },
  ok:    { bg: 'var(--st-ok-bg)',    bd: 'var(--st-ok-bd)',    fg: 'var(--st-ok-fg)' },
  none:  { bg: 'var(--st-none-bg)',  bd: 'var(--st-none-bd)',  fg: 'var(--st-none-fg)' },
};
...
function nameIncluded(csvName, clinicians, includeGroups, includeRoles) {
  const match = (clinicians || []).find((c) => matchesStaffMember(csvName, c));
  // Roles win when given: they are what the staff filter actually offers,
  // and "GP Registrar but not Medical Student" is a distinction the four
  // coarse groups cannot make. Groups stay the fallback so the stored
  // site-staffing config keeps working untouched.
  if (Array.isArray(includeRoles) && includeRoles.length) {
    return includeRoles.includes(match?.role || 'Unspecified');
  }
...
export function getSiteStaffingForDate(huddleData, csvDateStr, {
  ...
  for (const name of names) {
    if (!nameIncluded(name, clinicians, includeGroups, includeRoles)) continue;
  ...
  for (const site of sites) {
    ...
    for (const [name, rows] of Object.entries(rowsByName)) {
      ...
      for (const r of here) {
        ...
        if (!b.byName[name]) b.byName[name] = { name, initials: initialsFor(name, clinicians), urgent: 0, routine: 0, other: 0, types: {} };
        const entry = b.byName[name];
        const type = r.slotType || 'Unspecified';
        entry.types[type] = (entry.types[type] || 0) + 1;
        if (urgentTypes.has(r.slotType)) { entry.urgent += 1; b.urgent += 1; }
        else if (routineTypes.has(r.slotType)) { entry.routine += 1; b.routine += 1; }
        else entry.other += 1;
      }
    }
    ...
    for (const k of keys) {
      ...
      const clins = Object.values(b.byName).map((c) => ({
        ...
        // Duty first - they are the session's answer to "who do I call".
        // Then everyone offering slots, then those offering none, which is
        // where the divider falls.
        (y.duty ? 1 : 0) - (x.duty ? 1 : 0)
        || (y.offering ? 1 : 0) - (x.offering ? 1 : 0)
        || x.initials.localeCompare(y.initials));
