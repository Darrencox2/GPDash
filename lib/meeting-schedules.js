...
// Format the LOCAL calendar date. Every Date in this module is built as a
// local midnight (new Date(y, m, d) / 'T00:00:00'), and toISOString() converts
// to UTC before slicing — so through BST this returned 23:00 the previous day
// and every generated date was one day early. A "weekly Wednesday" schedule
// produced Tuesdays from late March to late October, then silently corrected
// itself at the October clock change, which is the reason a series could
// appear to move mid-run.
function toIso(d) {
...
export function generateOccurrences(schedule, count = 12, fromIso = null) {
  ...
  // Default "from" is local today. Deriving it from toISOString() meant that
  // between midnight and 01:00 BST the generator thought it was still
  // yesterday, and produced an occurrence that had already passed.
  const now = new Date();
  const from = fromIso
