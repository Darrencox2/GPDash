...
export async function resolvePracticeIdentifier(supabase, identifier) {
  ...
  if (UUID_RE.test(id)) {
    const { data } = await supabase
      .from('practices')
      .select('id, name, slug, ods_code, region, postcode, list_size, online_consult_tool, setup_completed_at, latitude, longitude, admin_district, buddy_cover_public, data_version')
      .eq('id', id)
      .maybeSingle();
    if (data) return data;
  }
...
export function adaptToV3Shape(v4Data) {
  ...
  return {
    ...
    // Staff Changes plan: planned people + timeline events. Stored like the
    // other planning blobs; real people are derived live, never stored.
    staffPlan: settings.extras?.staffPlan || null,
