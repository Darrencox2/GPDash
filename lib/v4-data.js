...
export async function loadPracticeData(supabase, practiceId) {
  ...
  const [
    ...
    supabase.from('working_patterns')
      ...
      .is('effective_to', null), // only current patterns
...
export function adaptToV3Shape(v4Data) {
  ...
  return {
    ...
    // For now, daily overrides, allocation history, rota notes — load separately
    dailyOverrides: settings.extras?.legacyDailyOverrides || {},
