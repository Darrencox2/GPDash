...
export function syncDailyOverrideOps(supabase, clinicianIds, oldMap, newMap) {
  const ops = [];
  ...
  if (!changed.size) return ops;

  const scoped = [...ids];
  for (const date of changed) {
    ops.push(supabase.from('daily_overrides').delete().eq('date', date).in('clinician_id', scoped));
  }
  ...
  if (rows.length) ops.push(supabase.from('daily_overrides').upsert(rows, { onConflict: 'clinician_id,date' }));
  return ops;
}
...
export function adaptToV3Shape(v4Data) {
  ...
  return {
    ...
    // Daily overrides now come from the daily_overrides TABLE, rebuilt into
    // the v3 shape the app reads. The extras blob remains the fallback for
    // any practice whose rows have not been backfilled yet, and is still
    // written alongside the table for one release as a safety net.
