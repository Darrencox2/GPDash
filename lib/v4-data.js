...
export async function resolvePracticeIdentifier(supabase, identifier) {
  ...
  if (UUID_RE.test(id)) {
    const { data } = await supabase
      .from('practices')
      .select('id, name, slug, ods_code, region, postcode, list_size, online_consult_tool, setup_completed_at, latitude, longitude, admin_district')
      .eq('id', id)
      .maybeSingle();
    if (data) return data;
  }
...
export async function loadPracticeData(supabase, practiceId, opts = {}) {
  ...
  const queries = [
    ...
    supabase.from('clinicians')
      .select('id, name, title, initials, role, group_id, status, sessions, buddy_cover, can_provide_cover, show_whos_in, aliases, linked_user_id, metadata')
      .eq('practice_id', practiceId)
      .order('name'),
...
export function adaptToV3Shape(v4Data) {
  ...
  // between the two here. Before this fix the adapter was reading
  // pattern["Monday"] which is always undefined → everyone showed as
  // off on the buddy cover page.
  const DAY_KEY_MAP = {
  ...
  for (const wp of v4Data.workingPatterns || []) {
    ...
    for (const [shortKey, longKey] of Object.entries(DAY_KEY_MAP)) {
      ...
      if (day?.am === 'in' || day?.pm === 'in') {
        weeklyRota[longKey].push(wp.clinician_id);
      }
    }
  }
