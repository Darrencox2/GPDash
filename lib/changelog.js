...
export const CHANGELOG = [
  {
    version: '4.167.0',
    date: '2026-09-15',
    title: 'Every headline number says what it is a ratio of, and the page reads to a screen reader',
    changes: [
      { type: 'fix', text: 'The wall board and Today disagreed about the same morning. The board said 85 per cent where Today said 94, 765 routine slots where Today said 649, and 10 clinicians where Today said 19. Two of those were real differences that nothing on screen explained, and one was simply wrong: the board tile labelled Routine 28 days was adding up 30 days of them' },
      { type: 'fix', text: 'The coverage dial had no caption at all, so the largest number on the page did not say what it was a ratio of. It now prints the count and the target under the needle and names where that target came from, predicted demand or the expected capacity table, which is the whole of the difference between the two readings' },
      { type: 'fix', text: 'The dial also used its own set of band thresholds, so 80 per cent was called Good there and Tight on the bar directly below it. Both now use the same scale. On top of that the dial averaged the two sessions, so a comfortable morning could hide a short afternoon: it keeps the average as its number but takes its colour and its word from the worse of the two. A headline greener than its own detail is the one that gets acted on' },
      { type: 'fix', text: 'Clinicians today means two different things in two places. On Today it counts clinicians offering bookable slots; on the board it counts who is in the building. Both said of 39 active and nothing else. Each now names which question it is answering' },
      { type: 'improvement', text: 'The Today page had exactly one heading in the whole of it, the word Today. Urgent on the day, Who is in today, Routine GP wait times, Routine Capacity, Morning, Afternoon and the rest were all plain text, so a screen reader user had no heading navigation and had to arrow through 3600 pixels in a straight line. They are real headings now, which changes nothing on screen' },
      { type: 'improvement', text: 'Added a skip link, and the sidebar now marks its active row with aria-current. Every section change used to re-present eleven nav buttons with no way past them and nothing saying which one you were on' },
      { type: 'fix', text: 'Toasts were never announced. That included the save conflict message, so a screen reader user was told nothing at all when an edit was refused' },
      { type: 'fix', text: 'A refused save is now a banner that stays, with a Reload button, carrying the whole explanation. It used to be a toast that dropped the second half of the sentence and left after twelve seconds, which is the one state on the page that actually requires an action' },
      { type: 'fix', text: 'Site and role badges were white text on the brand colours: 1.98 to 1 on Banwell lime, 2.15 on amber, 2.80 on Locking orange, against a floor of 4.5. These are the 22 by 13 chips that say which building a colleague is in, and two clinicians here share initials. The hue is the identity and has not moved; the ink now picks whichever of dark or white reads on it. Winscombe violet sits exactly on the crossover and could not clear the floor either way, so for that one the fill is a shade deeper' },
      { type: 'fix', text: 'Off today and the hide control were spans inside a button. That is invalid, and it meant every clinician announced as Off today X ME Madeleine Edwards Salaried GP 9 B, the destructive verb first. They are real buttons beside the card now, reachable by keyboard, visible on focus as well as hover so they exist on a phone, and at least 24 by 24 rather than 11 by 20' },
      { type: 'improvement', text: 'Everything now honours prefers reduced motion. Nothing did: not the slide and fade keyframes, not the transition on every nav row and person card, not the dial sweep, and not the progress bar, which loops forever' },
      { type: 'fix', text: 'Sidebar section labels were 2.36 to 1 at 11 pixels. They now clear the contrast floor against both ends of the sidebar gradient' },
      { type: 'fix', text: 'The buddy cover attendance grid stops at two columns until 1400 pixels. At three it was cutting every name to about five characters on the page whose entire job is saying who is covering whom' },
      { type: 'tweak', text: 'CCAS, NHS, PCN and EMIS join CPD and the rest in surviving sentence casing, so they stop appearing as Ccas and Emis' },
    ]
  },
  {
    version: '4.166.0',
    date: '2026-09-15',
    title: 'Today opens on the morning question instead of the summary',
    changes: [
      { type: 'improvement', text: 'Urgent on the day now leads the Today page. The morning and afternoon bars, their targets and the duty doctor sit in the first screen rather than just past it. On a 1440 by 900 laptop that section used to begin 21 pixels below the fold, so the first thing anyone saw was a gauge and a national benchmark and the answer to are we short this morning was a scroll away. On a phone it now starts 144 pixels down rather than roughly 1400' },
      { type: 'improvement', text: 'The summary gauge and its four tiles moved below that section, and on the days with no notices they run as a single row rather than a 2 by 2 block beside an empty panel. The gauge is smaller. Together those take about 470 pixels out of the top of the page and 185 out of its total height' },
      { type: 'improvement', text: 'The noticeboard gives its column back on the days it has nothing in it. It was holding a 251 by 506 pixel empty rectangle in the best position on the page. It is now a single line under the summary with the add box inline, and it takes the full panel back the moment a notice exists' },
      { type: 'fix', text: 'The Post button on the noticeboard was drawn 27 pixels outside its own card, because the text box beside it would not shrink below the width of its own placeholder' },
      { type: 'fix', text: 'On a phone the noticeboard used to render first, so a duty GP opened the dashboard on an empty box with a text field in it. It was also first in the keyboard tab order on a desktop, ahead of any content. Both came from the summary and the noticeboard sitting in the markup in the opposite order to the one they are drawn in, corrected with a display override that only applied on wide screens. The markup order is now the real order and the override is gone' },
      { type: 'improvement', text: 'The NHS demand ribbon moved to the foot of the page. It is a monthly comparison against other practices, not a morning read, and it was taking 73 pixels of the first screen every day' },
      { type: 'fix', text: 'The Urgent on the day heading no longer wraps its slot filter onto a second line on a phone' },
    ]
  },
  {
    version: '4.165.2',
    date: '2026-09-13',
    title: 'Hovering a card on the right of the buddy board no longer takes the board down',
    changes: [
      { type: 'fix', text: 'Resting on a clinician card near the right of the buddy cover board replaced the whole section with an error page. The explanation tooltip measures where it actually landed and slides itself back on screen if it overhangs the edge, but its entrance animation was overriding that slide, so the measurement never changed, the correction was applied again and again, and React stopped the page rather than loop forever. The slide and the animation now sit on separate elements, so the correction takes effect on the first try' },
      { type: 'fix', text: 'The same correction now gives up after a few attempts whatever the browser reports, so a tooltip that cannot be placed perfectly sits slightly off rather than taking the section down with it' },
    ]
  },
  {
    version: '4.165.1',
