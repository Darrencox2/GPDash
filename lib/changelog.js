export const CHANGELOG = [
  {
    ...
    changes: [
      ...
      { type: 'feature', text: 'Rules implemented: weekday recurring urgent shortage (e.g. "Wednesdays consistently below target"); routine target streaks (3+ consecutive weeks below or above target); mismatched-capacity weeks (routine over target while urgent runs short — rebalance opportunity); single-clinician concentration (>40% of a weekday\\'s urgent capacity from one person — bus-factor risk); AM/PM imbalance per weekday (one half consistently runs hot, the other cold); worst upcoming day (high predicted demand + low cover); bank-holiday rebound risk (predicted spike days adjacent to BHs); embargoed slot overload (>30% of routine slots embargoed in a week)' },
