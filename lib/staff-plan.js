...
export function sessionsByMonth(person, events, months) {
  ...
  // A recorded join means they were not here before it — true whether the
  // row is a planned hire or a real clinician whose start was recorded.
  // Without this a linked starter reads as having worked all year.
  const joins = mine.some(e => e.type === 'join');
  const out = {};
  for (const mk of months) {
    let value = (person.kind === 'planned' || joins) ? 0 : person.sessions;
    let away = false;
    for (const e of mine) {
      if (e.month > mk) break;
      if (e.type === 'join') value = e.sessions ?? (person.kind === 'planned' ? value : person.sessions);
...
// The one rule for "which list size applies on this date": the nearest
// earlier published size (even from before the window), else the current
// registered size. The chart and the per-1,000 chips both use it, so they
// can never quietly disagree for the same month.
export function listSizeLookup(listSizeByMonth, currentListSize) {
  const keys = Object.keys(listSizeByMonth || {}).sort();
  return (date) => {
    const mk = String(date).slice(0, 7);
    let best = null;
    for (const k of keys) { if (k > mk) break; best = k; }
    return best ? listSizeByMonth[best] : (currentListSize || null);
  };
}
export function per1000ByMonth(totals, months, listSizeByMonth, currentListSize) {
  const sizeAt = listSizeLookup(listSizeByMonth, currentListSize);
  const out = {};
  for (const mk of months) {
    const size = sizeAt(mk);
...
export function suggestedEventsFromWindDowns(people, events) {
  ...
  for (const p of people) {
    ...
    // absenceStart ties the accepted event to the absence the wind-down
    // already put on the board, so the running-changes list shows ONE row
    // for it rather than the event and an unclaimed board absence both.
    if (wd.type === 'left') {
      const month = monthKey(wd.endDate);
      if (!have.has(`${p.id}|leave|${month}`)) {
        out.push({ personRef: p.id, personName: p.name, type: 'leave', month, absenceStart: wd.startDate || null, note: 'From buddy wind-down (leaving)' });
      }
    } else {
      const month = monthKey(wd.startDate || wd.endDate);
      const toMonth = monthKey(wd.endDate);
      if (!have.has(`${p.id}|temp_leave|${month}`)) {
        out.push({ personRef: p.id, personName: p.name, type: 'temp_leave', month, toMonth, reason: 'sick', startDate: wd.startDate || null, endDate: wd.endDate, absenceStart: wd.startDate || null, note: 'From buddy wind-down (long-term absence)' });
      }
    }
  }
  return out;
...
export function capacityTimeline(people, events, months) {
  ...
  const recs = (people || []).map((p) => {
    ...
    for (const e of mine) {
      ...
      if (e.type === 'join') {
        // No number on a real person's join means their rota sessions —
        // the shape a linked planned person leaves behind.
        const v = e.sessions ?? (p.kind === 'planned' ? null : (p.sessions || 0));
        if (v != null) base.push({ date: e.startDate || mStart, value: v, type: 'join' });
      } else if (e.type === 'change') {
        if (e.sessions != null) base.push({ date: e.startDate || mStart, value: e.sessions, type: 'change' });
    ...
    return { p, base, away, joins: mine.some((e) => e.type === 'join') };
  });

  const valueAt = (rec, date) => {
    for (const a of rec.away) if (date >= a.start && date <= a.end) return 0;
    let v = (rec.p.kind === 'planned' || rec.joins) ? 0 : (rec.p.sessions || 0);
...
// ─── What moved, not what it became ────────────────────────────────────────
// The grid square used to print the new absolute ("6"), which reads as if
// someone works six sessions THIS month rather than that they went UP by two.
// The chart already speaks in signed changes; this makes the grid agree.
// Returns { [eventId]: signed change } for join / change / leave events.
export function changeDeltas(people, events) {
  const out = {};
  for (const p of people || []) {
    const mine = (events || [])
      .map((e, i) => ({ e, i, date: e.startDate || `${e.month}-01` }))
      .filter((x) => x.e.personRef === p.id && (x.e.type === 'join' || x.e.type === 'change' || x.e.type === 'leave'))
      .sort((a, b) => a.date.localeCompare(b.date) || a.i - b.i);
    // A recorded join means they were not here before it — the same rule
    // sessionsByMonth and capacityTimeline apply.
    const joins = mine.some((x) => x.e.type === 'join');
    let value = (p.kind === 'planned' || joins) ? 0 : (p.sessions || 0);
    for (const { e } of mine) {
      if (e.type === 'join') {
        const v = e.sessions ?? (p.kind === 'planned' ? value : (p.sessions || 0));
        out[e.id] = v - value;
        value = v;
      } else if (e.type === 'change') {
        if (e.sessions == null) continue;
        out[e.id] = e.sessions - value;
        value = e.sessions;
      } else {
        out[e.id] = -value;
        value = 0;
...
// "+2" / "−1.5" / "±0" — the minus is a real minus sign, not a hyphen, so a
// fall in sessions is not mistaken for a bullet at 10px, and a change that
// changes nothing says so rather than printing a bare number that reads
// like a session count.
export function signedSessions(n) {
