...
// The report builder and the scheduled-report email both colour bars from
// these. Kept here rather than in either consumer so an emailed chart and
// the same chart on screen cannot end up different colours.
export const REPORT_PALETTE = ['#6366f1','#10b981','#f59e0b','#ef4444','#0ea5e9','#a78bfa','#ec4899','#14b8a6','#f97316','#84cc16'];
export const REPORT_SINGLE = '#6366f1';

// Tabular form of a report result: a header row followed by one row per
// group. Shared by the Copy / CSV buttons in the builder and by the CSV
// that scheduled report emails attach, so the file a manager downloads
// and the one that lands in their inbox cannot disagree.
//
// Values stay numeric where they are numeric; only ratios are fixed to
// one decimal, matching what the chart prints.
export function buildReportRows(result) {
  if (!result) return [];
  const header = ['Group'];
  ...
  const rows = result.groups.map(g => {
    const r = [g.label];
    if (result.hasSplit) {
      result.series.forEach(s => r.push(result.isRatio ? g.cells[s.key].value.toFixed(1) : g.cells[s.key].value));
    } else {
      r.push(result.isRatio ? g.value.toFixed(1) : g.value);
      if (result.isRatio) r.push(g.numerator, g.denominator);
    }
    return r;
  });
  return [header, ...rows];
...
// The one-line "what stands out" sentence shown above the chart on
// screen and in the emailed report. Returns null when nothing stands out
// enough to be worth a sentence — a report where everyone is level
// should say nothing rather than manufacture a headline.
//
// Split reports are excluded: with multiple series there is no single
// "highest" to talk about without picking a series arbitrarily.
export function reportInsight(result) {
  if (!result?.groups?.length || result.hasSplit) return null;
  const fmt = (v) => result.isRatio ? `${v.toFixed(1)}%` : `${Math.round(v)}`;
  const top = [...result.groups].sort((a, b) => b.value - a.value)[0];
  if (!top || top.value <= 0) return null;

  if (result.isRatio) {
    // Compare against the SAME baseline the chart draws its reference line
    // at: the pooled ratio, totalValue. Not valueAvg, the unweighted mean of
    // the row percentages.
    //
    // The two differ whenever people work different amounts — 3 duty sessions
    // out of 10 and 11 out of 32 are both one row, but only one of them is a
    // tenth of the practice's work — and using one here while drawing the
    // other on the chart put two different baselines in the same report with
    // nothing to tell them apart. The pooled figure is also the honest
    // benchmark for fairness: if the work were shared in proportion to
    // sessions worked, everyone would sit exactly on it.
    const fair = result.totalValue;
    const ratio = fair > 0 ? top.value / fair : 0;
    if (ratio >= 1.4) {
      return `${top.label} is highest at ${fmt(top.value)} — about ${ratio.toFixed(1)}× the fair share of ${fmt(fair)}.`;
    }
  } else {
    const share = result.totalNum > 0 ? (top.numerator / result.totalNum) * 100 : 0;
    if (share >= 25) {
      return `${top.label} accounts for ${share.toFixed(0)}% of the total (${fmt(top.value)} of ${result.totalNum}).`;
    }
  }
  return null;
}
