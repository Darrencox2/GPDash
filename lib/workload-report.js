...
export function buildSessionFacts(slotFacts) {
  // Aggregate slot-grain facts into one fact per (date, session, clinician
  // who worked that session). Fully generic — no practice-specific duty or
  // support heuristic. Each session fact carries the set of slot types it
  // contained, its urgent-slot count, and a flag for the busiest-urgent
  // session of that day/session (the generic on-call equivalent).
  ...
  for (const f of (slotFacts || [])) {
    ...
    if (!g) {
      g = {
        ...
        role: f.role, isSystem: f.isSystem, slotTypeSet: new Set(), urgentCount: 0, total: 0,
      };
      groups.set(key, g);
    }
...
// Back-compat: an old `kinds` array (worked|duty|support) maps duty/support
// to 'busiest'. `sessions` optionally restricts to am/pm.
function sessionMode(f) {
  if (f?.mode) return f.mode;
  if (f?.kinds && f.kinds.length) {
    if (f.kinds.includes('worked')) return 'worked';
    if (f.kinds.includes('duty') || f.kinds.includes('support')) return 'busiest';
  }
  return 'worked';
}
...
function describeSessionFilter(f) {
  const mode = sessionMode(f);
  let label;
  if (mode === 'busiest') label = 'most-urgent sessions';
...
export const PRESET_GROUPS = [
  {
    ...
    presets: [
      { id: 'duty-share', label: 'Duty sessions as a share of own sessions',
        description: 'For each clinician, the sessions where they ran the most urgent slots (the de-facto duty doctor) divided by the total sessions they worked. Shows how the duty load is shared fairly across the team.',
        config: { grain: 'sessions', num: { mode: 'busiest' }, denomMode: 'custom', denom: { mode: 'worked' }, groupBy: 'clinician', range: 'last8next8', chart: 'bars', colourMode: 'conditional', colourInvert: true } },
