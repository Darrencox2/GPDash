...
const MIN_SPAN_DAYS_FOR_TREND = 90;   // 3 months minimum for a meaningful growth slope.
                                       // Below this the regression catches short-term
                                       // fluctuation as "trend" and extrapolates wildly
                                       // (e.g. 20 days of data → +70 long-term trend).
...
export function recalibrateDemandModel(rows, holidayRanges = []) {
  ...
  // Span check: only trust the slope if we have ≥90 days of history. With
  // shorter windows the regression catches transient effects (a busy
  // fortnight, a slow week) as "growth" and extrapolates them forward.
  // Setting growthPerDay = 0 means future predictions get the baseline +
  // day-of-week + month effects without an extrapolation term — the user
  // can re-upload a longer history to enable the trend later.
  const spanDaysForTrend = xs[xs.length - 1] - xs[0];
  const growthPerDay = (spanDaysForTrend < MIN_SPAN_DAYS_FOR_TREND || sxx === 0)
    ? 0
    : sxy / sxx;
