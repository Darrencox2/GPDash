...
export function inferWeeklyRota({
  ...
  //
  // BUT: only warn for clinicians who actually NEED initials fallback.
  // If their name matches a CSV row directly, the auto-match works fine
  // and the warning is misleading noise ("auto-match skipped" — when
  // in fact it wasn't skipped, name matching handled it).
  //
  // The realistic case this surfaces in: two clinicians whose names
  // share initials letters in the CSV (e.g. Justin Grandison and Jane
  // Gomm both producing "JG"), where both clinicians are name-matchable
  // to their CSV rows directly. The shared "JG" variant goes in the
  // ambiguity set; the clinician whose stored initials happen to be JG
  // gets falsely flagged.
  const cliniciansWithNameMatch = new Set();
  for (const c of eligibleClinicians) {
    for (const csvName of csvNamesSeen) {
      if (matchesStaffMember(csvName, c)) {
        cliniciansWithNameMatch.add(c.id);
        break;
      }
    }
  }
  ...
  for (const c of eligibleClinicians) {
    ...
    if (!cInit || !ambiguousInitials.has(cInit)) continue;
    // If this clinician has a name match in the CSV, the auto-match
    // works regardless of the initials clash. Skip the warning.
    if (cliniciansWithNameMatch.has(c.id)) continue;
...
// ═══════════════════════════════════════════════════════════════════════════
// AM/PM-aware inference for working_patterns table (v4)
// ═══════════════════════════════════════════════════════════════════════════
//
// Same matching logic as inferWeeklyRota but tracks AM and PM appearances
// independently, so the output captures half-day patterns (e.g. "Tue AM
// only") which the day-level rota can't express.
//
// Returns one pattern object per matched clinician — only those with at
// least one weekday session above threshold. Clinicians with no CSV
// activity get no row (caller decides whether to create an all-off
// pattern or leave them alone; for first-time auto-generation we leave
// them so the user explicitly picks via the working-days grid).
//
// Days are returned as 3-letter lowercase keys (mon, tue, wed, thu, fri)
// to match the working_patterns.pattern jsonb schema.
//
// @param huddleData       Parsed CSV (parseHuddleCSV output)
// @param clinicians       Practice clinicians from DB
// @param plannedAbsences  Optional — leave-adjust the denominator
// @param includeOnlyBuddyCover  Default false (include everyone with activity)
//
// @returns
//   patterns: [{ clinicianId, name, initials, pattern: { mon:{am,pm}, ... } }]
//   weeksAnalysed
//   ambiguityWarnings: [{ name, initials, csvNames }]

export function inferAmPmPatterns({
  huddleData,
  clinicians,
  huddleSettings = {},
  plannedAbsences = [],
  ...
  if (!huddleData?.dates?.length) {
    return { patterns: [], weeksAnalysed: 0, ambiguityWarnings: [], error: 'No CSV data' };
  }
  const eligibleClinicians = (clinicians || []).filter(c => {
    if (c.status === 'left' || c.status === 'administrative') return false;
    if (includeOnlyBuddyCover && !c.buddyCover) return false;
  ...
  if (eligibleClinicians.length === 0) {
    return { patterns: [], weeksAnalysed: 0, ambiguityWarnings: [], error: 'No eligible clinicians' };
  }

  // Same date prep as inferWeeklyRota
  const todayMs = new Date().setHours(0, 0, 0, 0);
  const datesWithObjs = huddleData.dates
    .map(ds => ({ ds, d: parseHuddleDateStr(ds) }))
    .filter(({ d }) => d && !isNaN(d) && d.getTime() <= todayMs)
  ...
  const DAY_FROM_NUM = [null, 'mon', 'tue', 'wed', 'thu', 'fri', null]; // Sun=0 unused, Mon=1..Fri=5
  const datesByDayKey = { mon: [], tue: [], wed: [], thu: [], fri: [] };
  for (const { ds, d } of recent) {
    const key = DAY_FROM_NUM[d.getDay()];
    if (key) datesByDayKey[key].push(ds);
  }
  ...
  const wasOnLeave = (clinicianId, dateMs) => {
    const list = absencesByClinician[clinicianId];
    if (!list) return false;
    for (const a of list) if (dateMs >= a.start && dateMs <= a.end) return true;
    return false;
  };

  // Pre-compute CSV name set + ambiguity map (same as inferWeeklyRota)
  const csvNamesSeen = new Set();
  const initialsToCsvNames = new Map();
  for (const dateStr of recentDates) {
    const cap = getHuddleCapacity(huddleData, dateStr, huddleSettings);
  ...
  for (const c of eligibleClinicians) {
    ...
    // Track per-(day,session) appearance counts + available denominators
    const counts = {};
    const denom = {};
    ...
    for (const day of DAY_KEYS) {
      for (const dateStr of datesByDayKey[day]) {
        const dateMs = parseHuddleDateStr(dateStr).getTime();
        const onLeave = wasOnLeave(c.id, dateMs);
        if (!onLeave) {
          denom[day].am++;
        ...
        const hadAm = inAm && (inAm.available > 0 || inAm.booked > 0 || inAm.embargoed > 0);
        const hadPm = inPm && (inPm.available > 0 || inPm.booked > 0 || inPm.embargoed > 0);
        if (hadAm) counts[day].am++;
        if (hadPm) counts[day].pm++;
      }
    }

    // Build pattern: 'in' if appeared on >= 50% of available weeks for
    // that (day, session). Threshold favours staying off when in doubt —
    // user adjusts via the working-days grid.
    const pattern = {};
    let anyOn = false;
    for (const k of DAY_KEYS) {
      const am = denom[k].am > 0 && (counts[k].am / denom[k].am) >= 0.5 ? 'in' : 'off';
      const pm = denom[k].pm > 0 && (counts[k].pm / denom[k].pm) >= 0.5 ? 'in' : 'off';
      pattern[k] = { am, pm };
      if (am === 'in' || pm === 'in') anyOn = true;
    }
  ...
  // Ambiguity warnings (only for clinicians without name match — same
  // logic as inferWeeklyRota for consistency).
  const ambiguityWarnings = [];
  for (const c of eligibleClinicians) {
    const cInit = (c.initials || '').toUpperCase();
    if (!cInit || !ambiguousInitials.has(cInit)) continue;
