...
export function getSectionStatuses({
  ...
  return {
    ...
    // ─── Optional sections ────────────────────────────────────────────
    // Three states: 'done' (set up → green), or 'optional' (not set up
    // yet → neutral grey, NOT amber and NOT green). Optional-and-empty is
    // a legitimate end state, so we never nag with amber, but we also
    // don't pretend it's "looks good" with green. `complete` stays true
    // so the strip can still auto-hide once the required sections are done.
    teamnet: {
      complete: true,
      optional: true,
      state: hasTeamnet ? 'done' : 'optional',
