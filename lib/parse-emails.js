...
// Email shape check. Permissive — RFC 5322 is famously hard to regex
// fully, but this catches the obvious "user typed garbage" cases.
//
// Lives here rather than in lib/api-helpers.js because that module pulls
// in next/server, which client components must not import. api-helpers
// re-exports this so server code keeps its existing import path.
const STRICT_EMAIL_RE = /^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/;
export function isEmail(s) {
  return typeof s === 'string' && STRICT_EMAIL_RE.test(s) && s.length <= 254;
}
