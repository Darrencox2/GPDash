...
// Title-case a name: "PETER CHOATE" → "Peter Choate", "COX, Darren" → "Darren Cox"
export function titleCaseName(name) {
  ...
  // Only fix if the non-title parts are all uppercase (don't touch "Dr. Ruth Harrison")
  const words = n.split(/\s+/);
  const nonTitleWords = words.filter(w => !w.match(/^(dr\.?|mr\.?|mrs\.?|ms\.?|miss|prof\.?)$/i));
  const isAllCaps = nonTitleWords.length > 0 && nonTitleWords.every(w => w === w.toUpperCase() && w.length > 1);
  if (isAllCaps) {
...
export function matchesStaffMember(csvName, staffMember) {
  ...
  const csvSurname = csvNorm.split(/\s+/).pop();
  const regSurname = regNorm.split(/\s+/).pop();
  if (csvSurname && regSurname && csvSurname === regSurname && csvSurname.length >= 3) {
    // Also check first initial or first name
    const csvFirst = csvNorm.split(/\s+/)[0];
    const regFirst = regNorm.split(/\s+/)[0];
    if (csvFirst === regFirst) return true;
    if (csvFirst[0] === regFirst[0]) return true;
  }
  // Check aliases
  if (staffMember.aliases?.length > 0) {
    return staffMember.aliases.some(a => {
      ...
      const aSurname = aNorm.split(/\s+/).pop();
      return aSurname && csvSurname && aSurname === csvSurname && aSurname.length >= 3;
    });
  }
  return false;
}
