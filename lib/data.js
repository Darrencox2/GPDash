...
export function buddyDefaultsForRole(role) {
  switch (role) {
    ...
    case 'Salaried GP':
      return { buddyCover: true, canProvideCover: true };
    case 'GP Registrar':
    case 'ANP':
      return { buddyCover: true, canProvideCover: false };
    case 'Locum':
    default:
      // canProvideCover is meaningless when buddyCover is off, but we set
      // it to false anyway so the DB column reflects "not part of the
      // pool" cleanly — no half-state.
      return { buddyCover: false, canProvideCover: false };
  }
}
