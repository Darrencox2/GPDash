...
// THE authoritative "what do they ordinarily work" accessor, at
// Morning / Afternoon / Evening granularity. sessionRota is surfaced by
// the adapter from working_patterns (am/pm/eve per weekday). Fallback:
// day-level weeklyRota membership means a default M+A day - covers any
// row the adapter has not yet surfaced. One source informs everything:
// buddy status, cover, boards, mismatches, and locum spend all read this.
export function getScheduledSessions(data, clinicianId, dayName) {
  const sr = data?.sessionRota?.[clinicianId]?.[dayName];
  if (Array.isArray(sr)) return sr;
  const rota = data?.weeklyRota?.[dayName];
  const list = !rota ? [] : Array.isArray(rota) ? rota : Object.values(rota);
  return list.includes(clinicianId) ? ['M', 'A'] : [];
}
...
export function computeDayStatus(data, dateKey, dayName) {
  ...
  // ONE-SOURCE RULE, session-complete form: someone is absent for the
  // day only when leave covers EVERY session they ordinarily work. A
  // morning-only clinician with an AM TeamNet entry IS absent; an
  // AM+PM clinician with the same entry is in (working the afternoon).
  const ABS_SESSIONS = (a) => a.session === 'am' ? ['M'] : a.session === 'pm' ? ['A'] : ['M', 'A', 'E'];
  const hasPlannedAbsence = (cid, dk, dn) => {
    const scheduled = getScheduledSessions(data, cid, dn || dayName);
    if (!scheduled.length) return false;
    const covered = new Set();
    for (const a of plannedAbs) {
      if (a.clinicianId === cid && dk >= a.startDate && dk <= a.endDate) {
        ABS_SESSIONS(a).forEach((x) => covered.add(x));
      }
    }
    return scheduled.every((x) => covered.has(x));
  };
  const isAbsOnWorkingDate = (cid, dk, dn) => {
    const rota = ensureArr(data?.weeklyRota?.[dn]);
    if (!rota.includes(cid)) return false;
    return hasPlannedAbsence(cid, dk, dn);
  };
  ...
  // Compute scheduled and present - SESSIONS are the source: a clinician
  // is scheduled on a day iff they ordinarily work ANY session of it.
  const naturalScheduled = buddyClinicians
    .filter(c => getScheduledSessions(data, c.id, dayName).length > 0)
    .map(c => c.id);
