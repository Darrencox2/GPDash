...
export function computeDayStatus(data, dateKey, dayName) {
  ...
  const hasPlannedAbsence = (cid, dk) => plannedAbs.some(a =>
    a.clinicianId === cid && dk >= a.startDate && dk <= a.endDate
    // ONE-SOURCE RULE (user spec): present for ANY session of the day
    // counts as IN. A half-day TeamNet absence (session am/pm) therefore
    // does NOT mark the day absent - the chip on the buddy board shows
    // the half-day detail instead, and cover is not allocated for them.
