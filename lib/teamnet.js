...
// v4 Supabase-authed /api/v4/sync-teamnet endpoint (the v3 /api/sync-teamnet was removed in the 2026-08 spring clean)
...
export function parseTeamnetCalendar(icsText, clinicians) {
  ...
  for (const event of events) {
    ...
    // Half-day detection. TeamNet shows entries like "Fri 28 Aug
    // Afternoon" - someone off only the afternoon still works their
    // morning EMIS session, so a full-day absence would be wrong. Two
    // signals, summary words first, then any DTSTART clock time:
    let session = null; // null = full day
    if (event.startDate === event.endDate) {
      if (/\b(afternoon|pm)\b/i.test(summary)) session = 'pm';
      else if (/\b(morning|am)\b/i.test(summary)) session = 'am';
      else if (event.startHour != null) {
        if (event.startHour >= 12) session = 'pm';
        else if (event.endHour != null && event.endHour <= 13 && event.endHour > 0) session = 'am';
      }
    }
    if (session) {
      reason = reason.replace(/\b(morning|afternoon|am|pm)\b/gi, '').replace(/\s+/g, ' ').trim() || 'Leave';
    }
  ...
  for (const absence of absences) {
    const key = `${absence.clinicianId}-${absence.startDate}-${absence.endDate}-${absence.session || 'full'}`;
    if (!seen.has(key)) {
      seen.add(key);
...
function processKeyValue(event, key, value) {
  ...
  if (mainKey === 'SUMMARY') {
    ...
    event.startHour = parseICSHour(value);
    ...
    event.endHour = parseICSHour(value);
  }
}

// Clock hour from an ICS datetime (e.g. 20260828T130000 -> 13).
// Returns null for date-only values.
function parseICSHour(value) {
