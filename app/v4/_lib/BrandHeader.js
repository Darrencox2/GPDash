  ...
  return (
    ...
    >
      <svg width="34" height="34" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg" style={{ flexShrink: 0 }}>
        <rect width="36" height="36" rx="7.6" fill="#1e293b" stroke="var(--g-text-mute)" strokeWidth="0.5"/>
      ...
      {!compact && (
        <div style={{ display: 'flex', alignItems: 'baseline', lineHeight: 1, fontFamily: "var(--font-mono)" }}>
          <span style={{ fontSize: 17, fontWeight: 400, color: 'var(--state-ok)', opacity: 0.4 }}>[</span>
          <span style={{ fontSize: 17, fontWeight: 700, color: 'var(--g-text-max)' }}>GP</span>
          <span style={{ fontSize: 17, fontWeight: 400, color: 'var(--state-ok)', opacity: 0.4 }}>]</span>
          <span style={{ fontFamily: "var(--font-heading)", fontSize: 18, fontWeight: 200, color: 'var(--state-ok)', letterSpacing: '3px', marginLeft: 2 }}>DASH</span>
        </div>
      )}
      {subtitle && (
