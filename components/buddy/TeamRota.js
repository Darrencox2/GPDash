...
export default function TeamRota({ data, saveData, helpers, huddleData }) {
  ...
  // Session-level pattern (Morning / Afternoon / Evening) - the SHARED
  // engine also used by locum spend, so what shows here is what spend
  // judges "extra" against. Auto-generated from EMIS history; click a
  // segment to pin a correction; amber = the history is uncertain.
  const hsPat = data?.huddleSettings || {};
  ...
  return (
    <div className="space-y-6">
      ...
      {genReport?.summary && (() => {
        ...
        return (
          <div className="card p-4 bg-purple-50 border border-purple-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1.5 text-xs text-slate-400">
              {genReport.summary.map(s => (
                <div key={s.initials || s.name} className={`flex items-center gap-2 py-0.5 px-2 -mx-2 rounded ${s.incomplete ? 'bg-red-50' : s.isFallback ? 'bg-amber-50' : ''}`}>
                  ...
                  <span className={`flex-shrink-0 ${s.incomplete ? 'text-red-700 font-medium' : 'text-slate-400'}`}>{s.days}</span>
                  {s.incomplete && <span className="text-red-600 text-[11px] font-semibold uppercase ml-auto">data incomplete</span>}
                  {!s.incomplete && s.isFallback && <span className="text-amber-700 text-[11px] italic ml-auto" title="Inferred from recent activity rather than the full window">recent activity only</span>}
                  {!s.incomplete && !s.isFallback && s.matchedAs && <span className="text-slate-400 text-[11px] truncate ml-auto" title={s.matchedAs}>= {s.matchedAs}</span>}
                </div>
              ))}
            </div>
            <div className="mt-3 space-y-1 text-xs text-slate-400">
              <p>
                <span className="font-medium text-slate-400">Standard rule:</span> a clinician is marked working a day if they appeared in CSV data for that weekday in at least 50% of weeks they weren't on planned leave.
              </p>
              {fallbackCount > 0 && (
                <p>
      ...
      <div className="card p-5">
          ...
          <table className="w-full">
            ...
            <tbody>
                        ...
                        {SLOTS.map(sl => {
                          ...
                          return (
                            <button key={sl}
                              ...
                              className="w-4 h-4 rounded-[4px] text-[11px] font-bold leading-none"
              ...
              {buddyCoverClinicians.length === 0 && (
                <tr><td colSpan={DAYS.length + 1} className="text-center py-8 text-sm text-slate-400">
        ...
        <div className="mt-4 flex gap-4 text-xs text-slate-400"><span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-emerald-100"></span>Working</span><span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-slate-100"></span>Day off</span></div>
      </div>
    </div>
  );
}
