  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-5">
        ...
        <p className="text-xs text-slate-400 mb-3">Set expected urgent slots per session. These targets colour-code the Today page and Capacity Planning: green (≥90%), amber (80–89%), red (&lt;80%).</p>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-xs text-slate-400 uppercase"><th className="text-left py-2 font-medium w-24"></th>{['Monday','Tuesday','Wednesday','Thursday','Friday'].map(d => <th key={d} className="text-center py-2 font-medium px-2">{d.slice(0,3)}</th>)}</tr></thead>
            <tbody>
              {['am','pm'].map(session => (
      ...
      <div className="card p-5">
        ...
        <p className="text-xs text-slate-400 mb-3">Set a weekly target for routine appointment slots. Used in Capacity Planning to colour-code weekly routine totals.</p>
        <div className="flex items-center gap-3">
          <label className="text-sm font-medium text-slate-400">Target slots per week</label>
      ...
      <div className="card p-5">
        ...
        <p className="text-sm text-slate-400 mb-6">Adjust how workload is calculated when balancing buddy allocations.</p>
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg"><div><div className="text-sm font-medium text-slate-900">Absent (File & Action)</div><div className="text-xs text-slate-400 mt-0.5">Multiplier when covering absent clinician</div></div><div className="flex items-center gap-2"><input type="number" min="0.5" max="10" step="0.5" value={data.settings?.absentWeight || 2} onChange={e => updateSettings('absentWeight', e.target.value)} className="w-20 px-3 py-2 rounded-md border border-slate-300 text-sm text-center focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent" /><span className="text-sm text-slate-400">× sessions</span></div></div>
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg"><div><div className="text-sm font-medium text-slate-900">Day Off (View Only)</div><div className="text-xs text-slate-400 mt-0.5">Multiplier when viewing day-off results</div></div><div className="flex items-center gap-2"><input type="number" min="0.5" max="10" step="0.5" value={data.settings?.dayOffWeight || 1} onChange={e => updateSettings('dayOffWeight', e.target.value)} className="w-20 px-3 py-2 rounded-md border border-slate-300 text-sm text-center focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent" /><span className="text-sm text-slate-400">× sessions</span></div></div>
        </div>
      </div>
      <div className="card p-4 bg-slate-50 border-slate-200"><h3 className="text-xs font-medium text-slate-400 mb-1">How the algorithm works</h3><p className="text-xs text-slate-400 leading-relaxed"><strong>Round-robin first:</strong> Everyone gets 1 allocation before anyone gets 2. Primary buddy is tried first, then secondary, then any eligible clinician.<br/><br/><strong>Weighted tiebreaking:</strong> When multiple clinicians have same count, lowest weighted load wins. Load = (absent × {data.settings?.absentWeight || 2}) + (day-off × {data.settings?.dayOffWeight || 1}).</p></div>

      {/* Data Cleanup */}
      <div className="card p-5">
        <h2 className="text-base font-semibold text-slate-900 mb-2">Data Cleanup</h2>
        <p className="text-sm text-slate-400 mb-4">Clear specific data without resetting everything.</p>
        <div className="space-y-2">
          <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50">
            <div><div className="text-sm font-medium text-slate-400">Room allocation history</div><div className="text-xs text-slate-400">Saved room assignments for past dates</div></div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50">
            <div><div className="text-sm font-medium text-slate-400">Huddle CSV data</div><div className="text-xs text-slate-400">Parsed appointment data — will need re-upload</div></div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50">
            <div><div className="text-sm font-medium text-slate-400">Buddy allocation history</div><div className="text-xs text-slate-400">Past buddy cover assignments</div></div>
      ...
      <div className="card p-5 border-red-200">
        ...
        <p className="text-sm text-slate-400">Practice-wide reset is disabled in the multi-tenant version. To clear specific data, use the Team Members or settings pages instead.</p>
      </div>
    </div>
  );
}
