      ...
      <div className="px-4 py-2.5" style={{ background: 'rgba(15,23,42,0.85)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-slate-600">Working days until next available routine GP appointments</div>
          </div>
        </div>
      </div>
        ...
        {cards.map((c, i) => {
          ...
          return (
            <div key={i} className="p-3 sm:p-4 text-center">
              <div className="text-[11px] sm:text-xs text-slate-500 mb-1">{c.title}</div>
              <div className="font-mono-data text-2xl sm:text-3xl font-bold leading-none" style={{ color: f.colour }}>{f.num}</div>
              <div className="text-[11px] text-slate-600 mt-1">{f.label || c.sub}</div>
              {c.wait?.date && <div className="text-[11px] text-slate-500 mt-0.5">{c.wait.date.toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })}</div>}
            </div>
          );
        })}
      </div>
