...
const LiveClock = memo(function LiveClock() {
  ...
  return <div className="text-5xl font-light text-slate-300 tabular-nums">{time.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</div>;
});
...
export default function HuddleFullscreen({ data, huddleData, onExit }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const chartDays = [];
      for (let i = 14; i >= 1; i--) { const d = new Date(today); d.setDate(d.getDate()-i); const isWE = d.getDay()===0||d.getDay()===6; const dk = d.toISOString().split('T')[0]; const pred = isWE ? null : predictDemand(d, w?.[dk]||null); chartDays.push({ predicted: pred?.predicted||null, date: d, dk, dayName: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][d.getDay()], dayNum: d.getDate(), isPast: true, isToday: false, isBH: pred?.isBankHoliday||false, isWE, confidence: pred?.confidence||{low:null,high:null} }); }
      ...
      for (let i = 1; i <= 14; i++) { const d = new Date(today); d.setDate(d.getDate()+i); const isWE = d.getDay()===0||d.getDay()===6; const dk = d.toISOString().split('T')[0]; const pred = isWE ? null : predictDemand(d, w?.[dk]||null); chartDays.push({ predicted: pred?.predicted||null, date: d, dk, dayName: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][d.getDay()], dayNum: d.getDate(), isPast: false, isToday: false, isBH: pred?.isBankHoliday||false, isWE, confidence: pred?.confidence||{low:null,high:null} }); }
  ...
  useEffect(() => {
    ...
    const loadChart = async () => {
      ...
      const days = demandData.chartDays, todayIdx = days.findIndex(d => d.isToday);
      const isClosed = days.map(d => d.isWE||d.isBH), isBH = days.map(d => d.isBH);
      ...
      chartInstance.current = new window.Chart(chartRef.current, {
        type: 'line', data: { labels, datasets: [
          ...
          { data: values, borderWidth: 2.5, tension: 0.3, spanGaps: false, borderColor: '#38bdf8',
            ...
            pointBorderColor: ctx => ctx.dataIndex===todayIdx?'#fbbf24':'transparent', pointBorderWidth: ctx => ctx.dataIndex===todayIdx?4:0,
            segment: { borderColor: ctx => ctx.p0DataIndex<todayIdx?'#94a3b8':'#38bdf8', borderDash: ctx => ctx.p0DataIndex>=todayIdx?[5,4]:undefined },
          },
        ]},
        plugins: [
          { id: 'shade', beforeDraw(chart) { const ctx=chart.ctx,xs=chart.scales.x,ys=chart.scales.y,bw=(xs.getPixelForValue(1)-xs.getPixelForValue(0))*0.5; ctx.save(); for(let i=0;i<isClosed.length;i++){if(isClosed[i]){const x=xs.getPixelForValue(i);ctx.fillStyle=isBH[i]?'#1c1917':'#1e293b';ctx.fillRect(x-bw,ys.top,bw*2,ys.bottom-ys.top);if(isBH[i]){ctx.fillStyle='#f59e0b33';ctx.fillRect(x-bw,ys.top,bw*2,3);}}} ctx.restore(); }},
          { id: 'todayLine', afterDraw(chart) { const ctx=chart.ctx,x=chart.scales.x.getPixelForValue(todayIdx),y=chart.scales.y; ctx.save();ctx.beginPath();ctx.setLineDash([3,3]);ctx.strokeStyle='#f59e0b44';ctx.lineWidth=1;ctx.moveTo(x,y.top);ctx.lineTo(x,y.bottom);ctx.stroke();ctx.restore(); }}
        ],
        options: { responsive: true, maintainAspectRatio: false, animation: { duration: 1200 },
          plugins: { legend: { display: false }, tooltip: { enabled: false } },
          scales: { x: { ticks: { font:{size:10}, color: ctx => { if(isBH[ctx.index]) return '#f59e0b88'; if(isClosed[ctx.index]) return '#334155'; if(ctx.index===todayIdx) return '#f59e0b'; return '#64748b'; }, maxRotation:0 }, grid:{display:false} },
  ...
  const categories = useMemo(() => {
    const inP=[],leave=[],off=[];
    visibleStaff.forEach(p => { if(p.longTermAbsent||p.status==='longTermAbsent'){leave.push({person:p,reason:'LTA'});return;} if(absenceMap[p.id]){leave.push({person:p,reason:absenceMap[p.id]});return;} if(hasCSV&&csvPresentIds.has(p.id)){inP.push({person:p});return;} if(!hasCSV&&p.buddyCover&&ensureArray(data.weeklyRota?.[dayName])?.includes(p.id)){inP.push({person:p});return;} off.push({person:p}); });
  ...
  // Routine — replicate exactly from today page
  ...
  const PersonCard = ({ person, delay, reason }) => {
    ...
    return (<div className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border mb-1.5 fs-slidein ${roleBg}`} style={{animationDelay:`${delay}s`}}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-slate-500 truncate">{person.role || 'Staff'}{reason ? ` — ${reason}` : ''}</div>
      </div>
    </div>);
  };
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-[9999] flex flex-col overflow-hidden" style={{background:'#f1f5f9',fontFamily:"'DM Sans', system-ui, sans-serif"}}>
      ...
      <div className="flex items-center px-8 py-6 bg-white border-b border-slate-200 flex-shrink-0">
        <div className="flex items-center gap-5 flex-1">
          <div className="bg-emerald-500 rounded-xl px-6 py-3 text-center"><div className="text-5xl font-extrabold text-white leading-none">{today.getDate()}</div><div className="text-sm text-white/80 uppercase">{MONTH_SHORT[today.getMonth()]}</div></div>
          <div><div className="text-3xl font-bold text-slate-900">{dayName}</div><div className="text-lg text-slate-400">{today.toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}</div></div>
        </div>
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-emerald-500 fs-live-dot"/><span className="text-lg text-slate-400">Live</span></div>
          {tw && <span className="text-lg text-slate-500">{Math.round(tw.temp)}°C · Feels {Math.round(tw.feelsLike)}°C{tw.precipMm>0?` · ${Math.round(tw.precipMm)}mm`:''}</span>}
          <LiveClock />
          <button onClick={onExit} className="px-5 py-2.5 rounded-xl border border-slate-200 text-base text-slate-400 hover:text-slate-600 hover:bg-slate-50">Exit fullscreen</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 flex-1 gap-2.5 p-2.5 min-h-0">
          ...
          <div className="flex items-stretch flex-1">
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="grid grid-cols-5 divide-x divide-slate-800 border-t border-slate-800">
                {topFactors.map((f,i) => (<div key={i} className="py-2 px-2 text-center fs-fadein" style={{animationDelay:`${0.5+i*0.1}s`}}><div className="text-[9px] text-slate-500 uppercase truncate">{f.label}</div><div className={`text-lg font-bold ${f.effect>=0?'text-blue-400':'text-emerald-400'}`}>{f.effect>0?'+':''}{Math.round(f.effect)}</div><div className="text-[9px] text-slate-600 truncate">{f.desc}</div></div>))}
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col">
          ...
          <div className="flex-1 flex flex-col p-3 overflow-hidden">
            {/* Gauge row */}
            <div className="flex items-center justify-center gap-6 mb-3 flex-shrink-0">
              {[{val:urgentAm,max:expectedAm||(urgentAm+bookedAm),pct:rawPctAm,label:'Morning',colour:pctColour(rawPctAm)},
                {val:urgentTotal,max:expectedTotal||(urgentTotal+bookedAm+bookedPm),pct:rawPctTotal,label:'Total',colour:pctColour(rawPctTotal),big:true},
                {val:urgentPm,max:expectedPm||(urgentPm+bookedPm),pct:rawPctPm,label:'Afternoon',colour:pctColour(rawPctPm)}
              ].map((g,gi) => {
                const sz = g.big ? 100 : 80, r2 = (sz-8)/2, cx2 = sz/2, cy2 = sz/2, circ2 = 2*Math.PI*r2;
                ...
                return (
                  <div key={gi} className="text-center fs-fadein" style={{animationDelay:`${0.1+gi*0.15}s`}}>
                    <svg width={sz} height={sz} viewBox={`0 0 ${sz} ${sz}`}>
                      <circle cx={cx2} cy={cy2} r={r2} fill="none" stroke="#f1f5f9" strokeWidth={8}/>
                      <circle cx={cx2} cy={cy2} r={r2} fill="none" stroke={g.colour} strokeWidth={8} strokeLinecap="round"
                      ...
                      <text x={cx2} y={g.big?cy2-2:cy2-1} textAnchor="middle" fill="#0f172a" style={{fontSize:g.big?'28px':'20px',fontWeight:800}}>{g.val}</text>
                      <text x={cx2} y={g.big?cy2+14:cy2+12} textAnchor="middle" fill="#94a3b8" style={{fontSize:'10px'}}>{g.label}</text>
                      {hasTarget && <text x={cx2} y={g.big?cy2+26:cy2+24} textAnchor="middle" fill={g.colour} style={{fontSize:'9px',fontWeight:600}}>{Math.round(g.pct)}%</text>}
                    </svg>
                  </div>
                );
              })}
            ...
            <div className="flex-1 overflow-auto">
              <div className="grid grid-cols-2 gap-1.5">
                {mergedClinicians.slice(0,8).map((c,i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between px-3 py-2 bg-slate-50 rounded-lg border border-slate-200 fs-slidein" style={{animationDelay:`${0.5+i*0.06}s`}}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col">
          ...
          <div className="p-4 flex-1 overflow-hidden">
            <div className="grid grid-cols-3 gap-3 h-full">
              <div className="overflow-hidden"><div className="text-xs text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-blue-500"/> Clinicians <span className="text-slate-300">{gpTeam.length}</span></div>{gpTeam.map((e,i) => <PersonCard key={e.person.id} person={e.person} delay={0.1+i*0.05}/>)}</div>
              <div className="overflow-hidden"><div className="text-xs text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-emerald-500"/> Nursing <span className="text-slate-300">{nursingTeam.length}</span></div>{nursingTeam.map((e,i) => <PersonCard key={e.person.id} person={e.person} delay={0.15+i*0.05}/>)}{othersTeam.length>0 && <><div className="text-xs text-slate-400 uppercase tracking-wider mt-3 mb-2 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-purple-500"/> Others <span className="text-slate-300">{othersTeam.length}</span></div>{othersTeam.map((e,i)=><PersonCard key={e.person.id} person={e.person} delay={0.3+i*0.05}/>)}</>}</div>
              <div className="overflow-hidden"><div className="text-xs text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-red-500"/> Absent <span className="text-slate-300">{categories.leaveAbsent.length}</span></div>{categories.leaveAbsent.map((e,i) => <PersonCard key={e.person.id} person={e.person} delay={0.2+i*0.05} reason={e.reason}/>)}{categories.leaveAbsent.length===0 && <div className="text-sm text-slate-300 px-3">None</div>}{categories.dayOff.length>0 && <div className="text-xs text-slate-300 mt-4">+ {categories.dayOff.length} day off</div>}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col">
          ...
          <div className="p-3 flex-1 flex flex-col gap-2.5 overflow-hidden">
            <div className="flex justify-center gap-4 flex-shrink-0">{routineGauges.map((g,i) => <GaugeSVG key={i} pct={g.pct} colour={g.colour} label={g.label} delay={i}/>)}</div>
            {cardData.length>0 && <div className="flex gap-3 flex-shrink-0">{cardData.map((c,i) => (
              <div key={c.id} className="flex-1 bg-slate-50 rounded-xl border border-slate-200 p-3 text-center fs-fadein" style={{animationDelay:`${0.6+i*0.1}s`}}>
                <div className="text-sm font-semibold text-slate-600">{c.title}</div>
                <div className="flex justify-center gap-2 mt-2">
                  <div className="text-center"><div className="bg-emerald-100 text-emerald-800 px-3 py-1 rounded-lg text-lg font-bold">{c.avail}</div><div className="text-[9px] text-slate-400 mt-0.5">available</div></div>
                  <div className="text-center"><div className="bg-amber-100 text-amber-800 px-3 py-1 rounded-lg text-lg font-bold">{c.emb}</div><div className="text-[9px] text-slate-400 mt-0.5">embargoed</div></div>
                  <div className="text-center"><div className="bg-slate-100 text-slate-600 px-3 py-1 rounded-lg text-lg font-bold">{c.booked}</div><div className="text-[9px] text-slate-400 mt-0.5">booked</div></div>
                </div>
                <div className="text-[10px] text-slate-400 mt-1">Next 7 days</div>
              </div>
            ))}</div>}
            <div className="flex-1 flex flex-col min-h-0">
              <div className="flex justify-between mb-1"><span className="text-[10px] text-slate-400">All routine · 30 days</span><div className="flex gap-2 text-[9px] text-slate-400"><span className="flex items-center gap-1"><span className="w-2 h-1.5 rounded-sm bg-emerald-400"/>Avail</span><span className="flex items-center gap-1"><span className="w-2 h-1.5 rounded-sm bg-amber-300"/>Emb</span><span className="flex items-center gap-1"><span className="w-2 h-1.5 rounded-sm bg-slate-300"/>Bkd</span></div></div>
