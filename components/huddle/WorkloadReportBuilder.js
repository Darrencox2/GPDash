...
function ChipGroup({ options, selected, onChange, allLabel = 'Any', allowAll = true }) {
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap">
      {allowAll && (
        <button onClick={() => onChange([])} className="text-sm px-2 py-1 rounded-md"
      ...
      {options.map(o => {
        ...
        return (
          <button key={o.id} onClick={() => toggle(o.id)} className="text-sm px-2 py-1 rounded-md flex items-center gap-1"
  ...
  return (
    <div className="flex flex-wrap" style={{ background: 'rgba(0,0,0,0.25)', borderRadius: 6, padding: 2, gap: 2 }}>
      {options.map(o => {
        ...
        return (
          <button key={o.id} disabled={disabled} onClick={() => !disabled && onChange(o.id)} className="text-sm font-medium px-2.5 py-1 rounded"
...
function MultiSelect({ label, options, selected, onChange }) {
  ...
  return (
    <div>
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between text-sm rounded-md px-2 py-1.5"
      ...
      {open && (
        <div className="mt-1 rounded-md p-2 space-y-1" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="flex items-center gap-1">
            <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search…" className="flex-1 text-sm rounded px-2 py-1"
              style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: '#e2e8f0', outline: 'none' }} />
            {sel.length > 0 && <button onClick={() => onChange([])} className="text-sm text-slate-400 px-1">Clear</button>}
          </div>
          <div className="max-h-40 overflow-y-auto space-y-0.5">
            ...
            {filtered.length === 0 && <div className="text-sm text-slate-600 px-1 py-1">No matches</div>}
          </div>
        </div>
      )}
    </div>
    ...
    <div className="space-y-2.5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="flex items-center justify-center text-xs font-bold rounded-full" style={{ width: 16, height: 16, background: 'rgba(99,102,241,0.25)', color: '#c7d2fe' }}>{n}</span>
  ...
  return (
    <div>
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-2 text-sm font-semibold text-slate-300" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
        <span className="text-slate-500">{open ? '▾' : '▸'}</span>
        <span>{title}</span>
        {badge ? <span className="text-xs px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(99,102,241,0.25)', color: '#c7d2fe' }}>{badge}</span> : null}
      </button>
  ...
  if (!huddleData) {
    return (
      <div className="rounded-xl p-12 text-center" style={{ background: 'rgba(15,23,42,0.7)', border: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <h3 className="text-base font-semibold text-slate-300 mb-1">No CSV data</h3>
        <p className="text-sm text-slate-400">Upload a huddle CSV on the Today page to build reports.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col lg:flex-row gap-4 items-start">
      ...
      <div className="flex-1 min-w-0 w-full rounded-xl p-5" style={{ background: 'rgba(15,23,42,0.55)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="flex items-start justify-between mb-3 flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <div className="text-right mr-1"><div className="text-sm text-slate-500 leading-none">Overall</div><div className="text-lg font-bold text-indigo-300 leading-tight">{fmt(result.totalValue)}</div></div>
            <button onClick={copyTable} className="text-sm px-2 py-1 rounded-md" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8' }}>Copy</button>
            <button onClick={downloadCsv} className="text-sm px-2 py-1 rounded-md" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8' }}>CSV</button>
          </div>
        </div>
        ...
        {dutyMissing ? (
          <p className="text-base text-slate-400 text-center py-8">Duty doctor slot not configured. Set a duty slot type in the Today page filter to enable duty &amp; support session reports.</p>
        ) : result.groups.length === 0 ? (
          <p className="text-base text-slate-400 text-center py-8">No data matches. Widen the date range, relax the filters, or turn off &ldquo;exclude system rows&rdquo;.</p>
      ...
      <div className="w-full lg:w-80 lg:flex-shrink-0 rounded-xl p-4 space-y-5" style={{ background: 'rgba(15,23,42,0.55)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <PanelSection title="Quick reports" right={
          canEdit ? <button onClick={() => setShowSaveBox(s => !s)} className="text-sm px-2 py-0.5 rounded" style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.3)', color: '#6ee7b7' }}>+ Save</button> : null
        }>
          {showSaveBox && canEdit && (
            <div className="flex items-center gap-1 mb-2">
              <input value={newReportName} onChange={e => setNewReportName(e.target.value)} placeholder="Report name…" className="flex-1 text-sm rounded px-2 py-1"
                style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: '#e2e8f0', outline: 'none' }} />
              <button onClick={saveReport} disabled={savingReport || !newReportName.trim()} className="text-sm px-2 py-1 rounded"
          ...
          {savedReports.length > 0 && (
            <div className="mb-2">
              <div className="text-xs uppercase tracking-wide text-emerald-600/80 mb-1">My saved reports</div>
              <div className="flex flex-wrap gap-1.5">
                {savedReports.map(r => (
                  <span key={r.id} className="flex items-center text-sm rounded-md overflow-hidden" style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.25)' }}>
          ...
          <div className="space-y-3">
            {PRESET_GROUPS.map(g => (
              <div key={g.group}>
                <div className="text-xs uppercase tracking-wide text-slate-600 mb-1">{g.group}</div>
                <div className="flex flex-wrap gap-1.5">
                  {g.presets.map(p => (
                    <button key={p.id} onClick={() => applyPreset(p)} className="text-sm px-2 py-1 rounded-md text-left"
        ...
        <StepSection n="1" title="Measure">
          ...
          <p className="text-xs text-slate-600 leading-snug">{isSession ? 'A session = one clinician working an AM or PM.' : 'A slot = one appointment slot.'}</p>
          ...
          <select value={denomMode} onChange={e => setDenomMode(e.target.value)} className="w-full text-sm rounded-md px-2 py-1.5"
        ...
        <StepSection n="2" title="Break down">
          <div className="text-xs text-slate-600">Group by (bars / rows)</div>
          <select value={groupBy} onChange={e => setGroupBy(e.target.value)} className="w-full text-sm rounded-md px-2 py-1.5"
            style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.1)', color: '#e2e8f0', outline: 'none' }}>
            {groupByOpts.map(o => <option key={o.id} value={o.id} style={{ background: '#1e293b' }}>{o.label}</option>)}
          </select>
          <div className="text-xs text-slate-600 mt-1">Compare by (splits into series)</div>
          <select value={splitBy} onChange={e => setSplitBy(e.target.value)} className="w-full text-sm rounded-md px-2 py-1.5"
            style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.1)', color: '#e2e8f0', outline: 'none' }}>
        ...
        <StepSection n="3" title="Filter">
          <Collapsible title="Narrow the data" badge={filterCount || null} defaultOpen={filterCount > 0}>
            ...
            {filterCount > 0 && <button onClick={() => setGlobalFilter({ clinicianIds: [], roles: [], locations: [], slotTypes: [], sessions: [] })} className="text-sm text-slate-400 hover:text-white mt-1" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Clear all filters</button>}
          </Collapsible>
        </StepSection>
        ...
        <StepSection n="4" title="View">
          ...
          <Collapsible title="More options">
            <div className="flex items-center justify-between"><span className="text-sm text-slate-400">Sort</span>
              <Segmented options={[{ id: 'value', label: 'Value' }, { id: 'alpha', label: 'A–Z' }]} value={sort} onChange={setSort} /></div>
            <div className="flex items-center justify-between"><span className="text-sm text-slate-400">Show</span>
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-slate-300">Exclude system rows (TRIAGE, CCAS…)</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={refOn} onChange={e => setRefOn(e.target.checked)} className="accent-indigo-500" />
              <span className="text-sm text-slate-300">Reference line</span>
            </label>
            {refOn && (
              <div className="pl-5 flex items-center gap-2">
                ...
                {refMode === 'custom' && <input value={refCustom} onChange={e => setRefCustom(e.target.value)} placeholder="value" className="w-16 text-sm rounded px-1.5 py-1" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', color: '#e2e8f0', outline: 'none' }} />}
              </div>
            )}
          </Collapsible>
        </StepSection>
    ...
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-xl overflow-hidden flex flex-col" style={{ background: 'linear-gradient(180deg,#1e293b,#0f172a)', border: '1px solid rgba(255,255,255,0.1)', maxHeight: '80vh' }}>
        <div className="px-4 py-3 flex items-center gap-2 border-b border-white/10 flex-shrink-0">
          <div>
            <div className="text-base font-semibold text-white">{groupLabel}{seriesLabel ? ` · ${seriesLabel}` : ''}</div>
            <div className="text-sm text-slate-500">{totalCount} {isSession ? 'session' : 'slot'}{totalCount === 1 ? '' : 's'} across {dates.length} day{dates.length === 1 ? '' : 's'}</div>
          </div>
        ...
        <div className="overflow-y-auto p-4 space-y-2">
          {dates.length === 0 && <p className="text-base text-slate-400 text-center py-6">No underlying records.</p>}
          {dates.map(iso => {
            ...
            return (
              <div key={iso} className="rounded-lg p-2" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                <div className="text-sm font-medium text-slate-300 mb-1">{d.toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' })}</div>
                <div className="flex flex-wrap gap-1">
                  {rows.map((f, i) => (
                    <span key={i} className="text-sm px-2 py-0.5 rounded" style={{ background: 'rgba(255,255,255,0.05)', color: '#94a3b8' }}>
    ...
    <div className="space-y-2">
      {multi && (
        <div className="flex flex-wrap gap-3 mb-2">
          {result.series.map((s, i) => <span key={s.key} className="flex items-center gap-1 text-sm text-slate-400"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: PALETTE[i % PALETTE.length] }} />{s.label}</span>)}
        </div>
      )}
      {result.groups.map((g, gi) => (
        <div key={g.key} className="flex items-center gap-3">
          <div className="w-32 lg:w-36 text-sm font-medium text-slate-300 truncate text-right" title={g.label}>{g.label}</div>
          <div className="flex-1 min-w-0">
            {multi ? (
                ...
                {result.series.map((s, si) => {
                  ...
                  return (
                    <div key={s.key} onClick={() => onPick && onPick(g.key, g.label, s.key, s.label)} className="relative h-3.5 rounded overflow-hidden cursor-pointer" style={{ background: 'rgba(255,255,255,0.05)' }} title="Click to drill down">
                      ...
                      <span className="absolute right-1 top-0 bottom-0 flex items-center text-[11px] text-slate-300">{fmt(cell.value)}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div onClick={() => onPick && onPick(g.key, g.label)} className="relative h-7 rounded-lg overflow-hidden cursor-pointer" style={{ background: 'rgba(255,255,255,0.06)' }} title="Click to drill down">
                ...
                <div className="absolute inset-0 flex items-center px-2.5">
                  ...
                  {isRatio && <span className="text-xs text-white/70 ml-2">{g.numerator}/{g.denominator}</span>}
                </div>
              </div>
            )}
          </div>
      ...
      {refValue != null && !multi && (
        <div className="flex items-center gap-2 pt-2 mt-1 text-sm text-slate-500" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        return (
          <div key={g.key} className="flex items-center gap-3">
            ...
            <div className="flex-1 relative h-7 rounded-lg overflow-hidden flex" style={{ background: 'rgba(255,255,255,0.06)' }}>
              ...
              <span className="absolute right-2 top-0 bottom-0 flex items-center text-sm font-bold text-white drop-shadow pointer-events-none">{fmt(total)}</span>
            </div>
          </div>
        );
      })}
...
function TrendView({ result, fmt, isRatio, refValue, refLabel, maxVal, onPick }) {
  ...
  return (
    <div className="w-full overflow-x-auto">
      {multi && <div className="flex flex-wrap gap-3 mb-2">{result.series.map((s, i) => <span key={s.key} className="flex items-center gap-1 text-sm text-slate-400"><span className="w-3 h-0.5" style={{ background: PALETTE[i % PALETTE.length] }} />{s.label}</span>)}</div>}
      ...
      <table className="w-full text-left" style={{ borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
            <th className="text-sm font-semibold uppercase tracking-wider text-slate-500 py-2 pr-4">{groupLabel}</th>
            {multi ? result.series.map(s => <th key={s.key} className="text-sm font-semibold uppercase tracking-wider text-slate-500 py-2 px-3 text-right">{s.label}</th>)
              : (<>{result.isRatio && <th className="text-sm text-slate-500 py-2 px-3 text-right uppercase tracking-wider font-semibold">Num</th>}{result.isRatio && <th className="text-sm text-slate-500 py-2 px-3 text-right uppercase tracking-wider font-semibold">Denom</th>}<th className="text-sm text-slate-500 py-2 pl-3 text-right uppercase tracking-wider font-semibold">{result.isRatio ? '%' : 'Count'}</th></>)}
          </tr>
        </thead>
        <tbody>
          {result.groups.map(g => (
